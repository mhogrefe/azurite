...
lemma pow_le_toNatLimbsList (l : List UInt64) (h_not_empty : l ≠ []) (hl : l.getLast? ≠ some 0) :
  ...
  have hz2 : toNatLimbsList [l.getLast h_not_empty] = (l.getLast h_not_empty).toNat := by
    ...
    simp
  have ht_last : l.getLast? = some (l.getLast h_not_empty) := List.getLast?_eq_getLast_of_ne_nil h_not_empty
  rw [ht_last] at hl
  ...
  exact Nat.le_add_right_of_le mul_bound
...
theorem compare_eq_compare (a b : AzNat) : compare a b = Ord.compare a.toNat b.toNat := by
