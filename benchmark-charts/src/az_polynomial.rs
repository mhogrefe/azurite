...
pub struct AzPolynomial<T> {
    pub coeffs: Vec<T>,
}

impl<T> AzPolynomial<T> {
    /// The zero polynomial.
    pub fn zero() -> Self {
        AzPolynomial { coeffs: Vec::new() }
    }
}

// ── SignificantBits ──────────────────────────────────────────────────────────

impl<'a, T> SignificantBits for &'a AzPolynomial<T>
where
    &'a T: SignificantBits,
{
...
impl<T: fmt::Display> fmt::Display for AzPolynomial<T> {
...
impl<T: ParseCoeff> FromStr for AzPolynomial<T> {
    type Err = String;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let s = s.trim();
        if s == "0" {
            return Ok(AzPolynomial::zero());
        }
...
mod tests {
    ...
    #[test]
    fn test_parse_zero() {
        let p: AzPolynomial<Integer> = "0".parse().unwrap();
        assert!(p.coeffs.is_empty());
    }

    #[test]
    fn test_parse_constant() {
        let p: AzPolynomial<Integer> = "42".parse().unwrap();
    ...
    fn test_parse_polynomial() {
        let p: AzPolynomial<Integer> = "3*x^2+2*x-5".parse().unwrap();
    ...
    fn test_parse_natural() {
        let p: AzPolynomial<Natural> = "3*x^2+2*x+5".parse().unwrap();
        assert_eq!(p.coeffs.len(), 3);
    ...
    fn test_parse_rational() {
        let p: AzPolynomial<Rational> = "1/2*x+3/4".parse().unwrap();
        assert_eq!(p.coeffs.len(), 2);
    }

    #[test]
    fn test_significant_bits() {
        let p: AzPolynomial<Natural> = "7*x+3".parse().unwrap();
    ...
    #[test]
    fn test_bare_x() {
        let p: AzPolynomial<Integer> = "x".parse().unwrap();
        assert_eq!(p.coeffs.len(), 2);
    ...
    #[test]
    fn test_negative_x() {
        let p: AzPolynomial<Integer> = "-x^2+1".parse().unwrap();
        assert_eq!(p.coeffs.len(), 3);
