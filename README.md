...
| `Equiv/GcdTower` | `map_toPoly_ngcdPoly`: UFD-generic one-level correctness of the tower gcd — under a bridge package (`ρ : D →+* M` injective, divisibility-reflecting, with `ρ ∘ ngcd = gcd`), `(toPoly (ngcdPoly P Q)).map ρ` is Mathlib's normalized `M[X]` gcd. Bundled as `NormalizedGcdBridge` with the induction step `NormalizedGcdBridge.step` (surjective bridge `D → M` ⟹ bridge `AzPolynomial D → M[X]`). Generalizes `Equiv/GcdInt`'s ℤ/ℚ argument to `M`/`Frac(M)`. |
