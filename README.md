...
| File | What it proves |
|------|----------------|
| `Equiv/Basic` | Round-trips `toRat_ofRat` / `ofRat_toRat`: `toRat`/`ofRat` form a bijection `AzRat ≃ ℚ`, including the canonical-zero and coprimality invariants; `toRat_injective` for transporting `ℚ`-side identities. |
