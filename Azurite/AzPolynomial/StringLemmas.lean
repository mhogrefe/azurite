...
/-!
# String Lemmas for `AzPolynomial`
...
- A `AzPolynomialToCharsValid` typeclass capturing the invariants, and generic
...
class NoDashInTail (R : Type _) [AzPolynomialToChars R] : Prop where
  no_dash_in_tail : ∀ (r : R), '-' ∉ (AzPolynomialToChars.toChars r).drop 1
...
lemma not_mem_tail_monomialToChars {R : Type _} [DecidableEq R] [Zero R] [dpc : AzPolynomialToChars R] [ndit : NoDashInTail R] (d : ℕ) (c : R) :
...
lemma mem_monomialToChars_nat_only_valid (d : ℕ) (c : ℕ) (ch : Char) (h : ch ∈ monomialToChars d c) :
  ...
  have h_toChars : AzPolynomialToChars.toChars c = natToChars c := rfl
...
lemma mem_monomialToChars_int_only_valid (d : ℕ) (c : ℤ) (ch : Char) (h : ch ∈ monomialToChars d c) :
  ...
  have h_toChars : AzPolynomialToChars.toChars c = intToChars c := rfl
  dsimp [monomialToChars] at h
  rw [h_toChars] at h
...
lemma mem_monomialToChars_rat_only_valid (d : ℕ) (c : ℚ) (ch : Char) (h : ch ∈ monomialToChars d c) :
  ...
  have h_toChars : AzPolynomialToChars.toChars c = ratToChars c := rfl
  dsimp [monomialToChars] at h
  rw [h_toChars] at h
  split_ifs at h with hc hd hc1 hd1 hd1_ignore
  · simp only [List.mem_singleton] at h
...
lemma mem_monomialToChars_zmod_only_valid {n : ℕ} [NeZero n] (d : ℕ) (c : ZMod n) (ch : Char) (h : ch ∈ monomialToChars d c) :
  ch = 'x' ∨ ch = '*' ∨ ch = '^' ∨ ('0'.toNat ≤ ch.toNat ∧ ch.toNat ≤ '9'.toNat) := by
  have h_toChars : AzPolynomialToChars.toChars c = natToChars c.val := rfl
  dsimp [monomialToChars] at h
  rw [h_toChars] at h
  split_ifs at h with hc hd hc1 hd1 hd1_ignore
  · simp only [List.mem_singleton] at h
...
@[simp] lemma parse_monomialToChars_zero {R : Type _} [AzPolynomialParsable R] [DecidableEq R] [Zero R] [AzPolynomialToChars R] (d : ℕ)
  (hparse : AzPolynomialParsable.parse ['0'] = some (0 : R)) :
...
@[simp] lemma parse_monomialToChars_ne_zero_nat (d : ℕ) (c : ℕ) (hc : c ≠ 0) :
  ...
  · -- d = 0
    ...
    rw [h_split]
    dsimp [AzPolynomialParsable.parse]
  ...
  · -- d ≠ 0
    ...
    · by_cases hd1 : d = 1
      · -- 1x
        have hc1_decide : (AzPolynomialToChars.toChars c == ['1']) = true := by
          rw [h_toChars, hc1]
          rfl
        simp [hc1_decide, hd1]
        dsimp [parseMonomial]
        ...
        rw [h_split]
        simp [AzPolynomialParsable.parse]
    ...
    · by_cases hd1 : d = 1
      · -- cx
        have hc1_decide : (AzPolynomialToChars.toChars c == ['1']) = false := by
          cases h : (AzPolynomialToChars.toChars c == ['1'])
          · rfl
          · exfalso
            have h_eq : AzPolynomialToChars.toChars c = ['1'] := eq_of_beq h
            rw [h_toChars] at h_eq
            exact hc1 h_eq
        have hc2_decide : (AzPolynomialToChars.toChars c == ['-', '1']) = false := by
          cases h : (AzPolynomialToChars.toChars c == ['-', '1'])
          · rfl
          · exfalso
            have h_eq : AzPolynomialToChars.toChars c = ['-', '1'] := eq_of_beq h
...
lemma parseMonomial_c_opt_int_pos (hd : List Char) (n : ℕ)
  (h : AzPolynomialParsable.parse (R := ℕ) (parseMonomial_c_opt_prefix hd) = some n) :
  AzPolynomialParsable.parse (R := ℤ) (parseMonomial_c_opt_prefix hd) = some (n : ℤ) := by
  dsimp [AzPolynomialParsable.parse] at h ⊢
...
lemma parseMonomial_int_pos_helper (cs : List Char) (d : ℕ) (n : ℕ) (h : parseMonomial (R := ℕ) cs = some (d, n)) :
  ...
  · rename_i hd tail
    ...
    · rw [h_split] at h
      dsimp at h ⊢
      cases hc : AzPolynomialParsable.parse (R := ℕ) hd
    ...
    · rename_i tail_head tail_tail
      ...
      · rw [h_split] at h
        ...
        · generalize h_match : parseMonomial_c_opt_prefix hd = hd_match at h ⊢
          cases h_parse : AzPolynomialParsable.parse (R := ℕ) hd_match
          ...
          · rename_i val_n
            ...
            change (AzPolynomialParsable.parse (R := ℤ) hd_match).bind (fun c => some (1, c)) = some (1, (val_n : ℤ))
            rw [hc_int]
            rfl
        · rename_i rest
          ...
          · rename_i val_d
            ...
            · rename_i val_n
              ...
              have hc_int := parseMonomial_c_opt_int_pos hd val_n hc_input
              rw [h_match] at hc_int
              change (Option.bind (some val_d) fun y => (AzPolynomialParsable.parse (R := ℤ) hd_match).bind fun c => some (y, c)) = some (val_d, (val_n : ℤ))
              rw [hc_int]
              rfl
        · contradiction
    ...
    · rename_i tail_head tail_tail
      ...
      · rw [h_split] at h
        ...
        · generalize h_match : parseMonomial_c_opt_prefix ('-' :: hd) = hd_match at ⊢
          ...
          cases h_parse : AzPolynomialParsable.parse (R := ℕ) hd_match_pos
          · rw [h_parse] at h; contradiction
          · rename_i val_n
            rw [h_parse] at h; dsimp at h
            injection h with heq; injection heq with hd_eq hn_eq
    ...
    · rename_i hd0
      ...
      · -- x^d or x
        ...
        · -- x
          ...
          rw [h_split]
          dsimp
          have h_parse : AzPolynomialParsable.parse (R := ℤ) (parseMonomial_c_opt_prefix []) = some 1 := rfl
          rw [h_parse]
          dsimp
          rw [hc1]
        · -- x^d
        ...
        · -- -x^d or -x
          ...
          · -- -x
            have hc1_decide : (AzPolynomialToChars.toChars c == ['1']) = false := by
              rw [h_toChars, hcn1]
              rfl
            have hc2_decide : (AzPolynomialToChars.toChars c == ['-', '1']) = true := by
              rw [h_toChars, hcn1]
              rfl
            simp [hc1_decide, hc2_decide, hd1]
            dsimp [parseMonomial]
        ...
        · by_cases hd1 : d = 1
          · -- cx
            have hc1_decide : (AzPolynomialToChars.toChars c == ['1']) = false := by
              ...
              · exfalso
                have h_eq : AzPolynomialToChars.toChars c = ['1'] := eq_of_beq h
                have h_parse : parseIntChars (AzPolynomialToChars.toChars c) = some c := by rw [h_toChars, parseIntChars_intToChars]
                rw [h_eq] at h_parse
            ...
            · have h_get : (intToChars c ++ ['*']).getLast? = some '*' := append_star_getLast? _
              ...
              simp_rw [if_pos h_get, h_drop]
              have h_parse : AzPolynomialParsable.parse (R := ℤ) (intToChars c) = some c := parseIntChars_intToChars c
              rw [h_parse]
              rfl
          · -- cx^d
            have hc1_decide : (AzPolynomialToChars.toChars c == ['1']) = false := by
...
lemma parseZModChars_of_parseNatChars {n : ℕ} [NeZero n] (cs : List Char) (v : ℕ)
  (h : AzPolynomialParsable.parse (R := ℕ) cs = some v) :
  AzPolynomialParsable.parse (R := ZMod n) cs = some (v : ZMod n) := by
  dsimp [AzPolynomialParsable.parse] at h ⊢
...
lemma parseMonomial_zmod_of_nat {n : ℕ} [NeZero n] (cs : List Char) (d : ℕ) (v : ℕ)
  ...
  · rename_i c_cs tail
    ...
    · rw [hsplit] at h
      dsimp at h ⊢
      cases hparse : AzPolynomialParsable.parse (R := ℕ) c_cs
    ...
    · rename_i head2 tail2
      ...
      · rw [hsplit] at h
        ...
        · generalize hpfx : parseMonomial_c_opt_prefix c_cs = pfx at h ⊢
          cases hparse : AzPolynomialParsable.parse (R := ℕ) pfx
          · rw [hparse] at h; contradiction
          · rename_i val
            rw [hparse] at h; dsimp at h
            injection h with heq
    ...
    · rename_i hd0
      ...
      · -- x^d or x
        ...
        · -- x
          ...
          rw [h_split]
          dsimp
          have h_parse : AzPolynomialParsable.parse (R := ℚ) (parseMonomial_c_opt_prefix []) = some 1 := rfl
          rw [h_parse]
          dsimp
          rw [hc1]
        · -- x^d
        ...
        · by_cases hd1 : d = 1
          · -- cx
            have hc1_decide : (AzPolynomialToChars.toChars c == ['1']) = false := by
              ...
              · exfalso
                have h_eq : AzPolynomialToChars.toChars c = ['1'] := eq_of_beq h
                have h_parse : parseRatChars (AzPolynomialToChars.toChars c) = some c := by rw [h_toChars, parseRatChars_ratToChars]
                rw [h_eq] at h_parse
            ...
            · have h_get : (ratToChars c ++ ['*']).getLast? = some '*' := append_star_getLast? _
              ...
              simp_rw [if_pos h_get, h_drop]
              have h_parse : AzPolynomialParsable.parse (R := ℚ) (ratToChars c) = some c := parseRatChars_ratToChars c
              rw [h_parse]
              rfl
          · -- cx^d
            have hc1_decide : (AzPolynomialToChars.toChars c == ['1']) = false := by
...
/-! ### `AzPolynomialParsableValid` typeclass -/

class AzPolynomialParsableValid (R : Type _) [DecidableEq R] [Zero R] [AzPolynomialToChars R] [AzPolynomialParsable R] : Prop where
  parse_monomialToChars_ne_zero : ∀ (d : ℕ) (c : R) (_ : c ≠ 0), parseMonomial (monomialToChars d c) = some (d, c)

export AzPolynomialParsableValid (parse_monomialToChars_ne_zero)

instance : AzPolynomialParsableValid ℕ where
  parse_monomialToChars_ne_zero := parse_monomialToChars_ne_zero_nat

instance : AzPolynomialParsableValid ℤ where
  parse_monomialToChars_ne_zero := parse_monomialToChars_ne_zero_int

instance : AzPolynomialParsableValid ℚ where
  parse_monomialToChars_ne_zero := parse_monomialToChars_ne_zero_rat

instance {n : ℕ} [NeZero n] : AzPolynomialParsableValid (ZMod n) where
  parse_monomialToChars_ne_zero := parse_monomialToChars_ne_zero_zmod

/-! ### `AzPolynomialToCharsValid` typeclass
...
/-- A typeclass asserting that `AzPolynomialToChars.toChars r` is never empty
    and never starts with `'0'` unless `r = 0`. -/
class AzPolynomialToCharsValid (R : Type*) [DecidableEq R] [Zero R] [AzPolynomialToChars R] : Prop where
  /-- The char representation is never the empty list. -/
  toChars_ne_nil : ∀ (r : R), AzPolynomialToChars.toChars r ≠ []
  /-- For nonzero `r`, the char representation does not start with `'0'`. -/
  toChars_not_start_zero : ∀ (r : R) (cs : List Char), r ≠ 0 → AzPolynomialToChars.toChars r ≠ '0' :: cs

instance : AzPolynomialToCharsValid ℕ where
  toChars_ne_nil c := natToChars_ne_nil c
  toChars_not_start_zero c cs hc := natToChars_not_start_zero c cs hc

instance : AzPolynomialToCharsValid ℤ where
  toChars_ne_nil c := intToChars_ne_nil c
  toChars_not_start_zero c cs hc := intToChars_not_start_zero c cs hc

instance : AzPolynomialToCharsValid ℚ where
  toChars_ne_nil c := ratToChars_ne_nil c
  toChars_not_start_zero c cs hc := ratToChars_not_start_zero c cs hc

instance {n : ℕ} [NeZero n] : AzPolynomialToCharsValid (ZMod n) where
  toChars_ne_nil c := by
...
/-- If `AzPolynomialToCharsValid R`, then `monomialToChars d c` is never `[]`. -/
lemma monomialToChars_ne_nil {R : Type*} [DecidableEq R] [Zero R] [AzPolynomialToChars R]
    [AzPolynomialToCharsValid R] (d : ℕ) (c : R) : monomialToChars d c ≠ [] := by
  dsimp [monomialToChars]; split_ifs
  · intro h; contradiction
  · exact AzPolynomialToCharsValid.toChars_ne_nil c
  · simp
  · simp
  · intro h; contradiction
  · intro h; contradiction
...
/-- If `AzPolynomialToCharsValid R` and `c ≠ 0`, then `monomialToChars d c` does not start with `'0'`. -/
lemma monomialToChars_not_start_zero {R : Type*} [DecidableEq R] [Zero R] [AzPolynomialToChars R]
    [AzPolynomialToCharsValid R] (d : ℕ) (c : R) (cs : List Char) (hc : c ≠ 0) :
    monomialToChars d c ≠ '0' :: cs := by
  dsimp [monomialToChars]; split_ifs
  · intro h; contradiction
  · exact AzPolynomialToCharsValid.toChars_not_start_zero c cs hc
  · simp
  · simp
  · simp
  · simp
  · rw [show AzPolynomialToChars.toChars c ++ ['*'] ++ ['x'] =
          AzPolynomialToChars.toChars c ++ ['*', 'x'] by simp]
    exact append_not_start_char _ _ '0'
      (AzPolynomialToCharsValid.toChars_ne_nil c)
      (fun cs => AzPolynomialToCharsValid.toChars_not_start_zero c cs hc) cs
  · rw [show AzPolynomialToChars.toChars c ++ ['*'] ++ 'x' :: '^' :: natToChars d =
          AzPolynomialToChars.toChars c ++ ('*' :: 'x' :: '^' :: natToChars d) by simp]
    exact append_not_start_char _ _ '0'
      (AzPolynomialToCharsValid.toChars_ne_nil c)
      (fun cs => AzPolynomialToCharsValid.toChars_not_start_zero c cs hc) cs
...
lemma monomials_mem {R} [DecidableEq R] [Semiring R] [AzPolynomialToChars R] (coeffs : List R) (m : List Char) :
...
lemma withSigns_mem {R} [DecidableEq R] [Semiring R] [AzPolynomialToChars R] (monomials : List (List Char)) (x : List Char) :
...
lemma toChars_ne_empty {R} [DecidableEq R] [Semiring R] [AzPolynomialToChars R] [AzPolynomialParsable R] [AzPolynomialParsableValid R]
  (p : AzPolynomial R)
  ...
  · rename_i h_p_ne_zero
    have h_back : p.coeffs.back? ≠ none := by
      ...
      have hpe : p.coeffs = (0 : AzPolynomial R).coeffs := by
        have hz : (0 : AzPolynomial R).coeffs = #[] := rfl
        rw [hz, hp]
      have hp0 : p = 0 := AzPolynomial.ext hpe
      contradiction
...
lemma toChars_nat_ne_empty (p : AzPolynomial ℕ) : toChars p ≠ "" :=
  toChars_ne_empty p monomialToChars_nat_ne_nil

lemma toChars_nat_not_start_zero_of_ne_zero (p : AzPolynomial ℕ) (hp : p ≠ 0) (cs : List Char) :
  ...
  · simp only [String.toList_ofList]
    ...
    have h_back : p.coeffs.back? ≠ none := by
      ...
      have hpe : p.coeffs = (0 : AzPolynomial ℕ).coeffs := by
        have hz : (0 : AzPolynomial ℕ).coeffs = #[] := rfl
        rw [hz, hp2]
      have hp0_eq : p = 0 := AzPolynomial.ext hpe
      contradiction
    have h_back2 : ∃ c, p.coeffs.back? = some c := by
      rcases Option.ne_none_iff_exists.mp h_back with ⟨c, hc_eq⟩
      exact ⟨c, hc_eq.symm⟩
...
lemma mem_toChars_nat_only_valid (p : AzPolynomial ℕ) (ch : Char) (h : ch ∈ (toChars p).toList) :
  ch = 'x' ∨ ch = '+' ∨ ch = '*' ∨ ch = '^' ∨ ('0'.toNat ≤ ch.toNat ∧ ch.toNat ≤ '9'.toNat) := by
  dsimp [toChars] at h
...
lemma toChars_int_not_start_zero_of_ne_zero (p : AzPolynomial ℤ) (hp : p ≠ 0) (cs : List Char) :
  (toChars p).toList ≠ '0' :: cs := by
  dsimp [toChars]
  split_ifs with hp0
  · contradiction
  · simp only [String.toList_ofList]
    ...
    have h_back : p.coeffs.back? ≠ none := by
      intro h_none
      have hp2 : p.coeffs = #[] := Array.back?_eq_none_iff.mp h_none
      have hpe : p.coeffs = (0 : AzPolynomial ℤ).coeffs := by
        have hz : (0 : AzPolynomial ℤ).coeffs = #[] := rfl
        rw [hz, hp2]
      have hp0_eq : p = 0 := AzPolynomial.ext hpe
      contradiction
    have h_back2 : ∃ c, p.coeffs.back? = some c := by
...
lemma toChars_int_ne_empty (p : AzPolynomial ℤ) : toChars p ≠ "" :=
  toChars_ne_empty p monomialToChars_int_ne_nil

lemma mem_toChars_int_only_valid (p : AzPolynomial ℤ) (ch : Char) (h : ch ∈ (toChars p).toList) :
  ch = 'x' ∨ ch = '+' ∨ ch = '-' ∨ ch = '*' ∨ ch = '^' ∨ ('0'.toNat ≤ ch.toNat ∧ ch.toNat ≤ '9'.toNat) := by
  dsimp [toChars] at h
  split_ifs at h with hp0
  · -- p = 0
...
lemma toChars_rat_not_start_zero_of_ne_zero (p : AzPolynomial ℚ) (hp : p ≠ 0) (cs : List Char) :
  (toChars p).toList ≠ '0' :: cs := by
  dsimp [toChars]
  split_ifs with hp0
  · contradiction
  · simp only [String.toList_ofList]
    ...
    have h_back : p.coeffs.back? ≠ none := by
      intro h_none
      have hp2 : p.coeffs = #[] := Array.back?_eq_none_iff.mp h_none
      have hpe : p.coeffs = (0 : AzPolynomial ℚ).coeffs := by
        have hz : (0 : AzPolynomial ℚ).coeffs = #[] := rfl
        rw [hz, hp2]
      have hp0_eq : p = 0 := AzPolynomial.ext hpe
      contradiction
    have h_back2 : ∃ c, p.coeffs.back? = some c := by
...
lemma toChars_rat_ne_empty (p : AzPolynomial ℚ) : toChars p ≠ "" :=
  toChars_ne_empty p monomialToChars_rat_ne_nil

lemma mem_toChars_rat_only_valid (p : AzPolynomial ℚ) (ch : Char) (h : ch ∈ (toChars p).toList) :
...
lemma toChars_zmod_not_start_zero_of_ne_zero {n : ℕ} [NeZero n] (p : AzPolynomial (ZMod n)) (hp : p ≠ 0) (cs : List Char) :
  (toChars p).toList ≠ '0' :: cs := by
  dsimp [toChars]
  split_ifs with hp0
  · contradiction
  · simp only [String.toList_ofList]
    ...
    have h_back : p.coeffs.back? ≠ none := by
      intro h_none
      have hp2 : p.coeffs = #[] := Array.back?_eq_none_iff.mp h_none
      have hpe : p.coeffs = (0 : AzPolynomial (ZMod n)).coeffs := by
        have hz : (0 : AzPolynomial (ZMod n)).coeffs = #[] := rfl
        rw [hz, hp2]
      have hp0_eq : p = 0 := AzPolynomial.ext hpe
      contradiction
    have h_back2 : ∃ c, p.coeffs.back? = some c := by
...
lemma toChars_zmod_ne_empty {n : ℕ} [NeZero n] (p : AzPolynomial (ZMod n)) : toChars p ≠ "" :=
  toChars_ne_empty p monomialToChars_zmod_ne_nil

lemma mem_toChars_zmod_only_valid {n : ℕ} [NeZero n] (p : AzPolynomial (ZMod n)) (ch : Char) (h : ch ∈ (toChars p).toList) :
  ch = 'x' ∨ ch = '+' ∨ ch = '*' ∨ ch = '^' ∨ ('0'.toNat ≤ ch.toNat ∧ ch.toNat ≤ '9'.toNat) := by
  dsimp [toChars] at h
  split_ifs at h with hp0
  · -- p = 0
...
lemma hasDuplicateExponents_false_of_listEnum {R} [Semiring R] [DecidableEq R] (p : AzPolynomial R) :
...
/-- Normalizing the coefficient array of an already-valid AzPolynomial is an identity. -/
private lemma normalize_idem {R} [Semiring R] [DecidableEq R] (p : AzPolynomial R) :
...
lemma listToPoly_indexed_nonzero {R} [Semiring R] [DecidableEq R] (p : AzPolynomial R) :
  ...
  · -- Case p = 0: listEnum [] = [], listToPoly [] = 0 = p
    have hp : p = 0 := AzPolynomial.ext (by simpa using hzero)
    subst hp
    simp [listToPoly, listEnum, listEnum.aux, AzPolynomial.ext_iff, normalize]
...
/-! ### Bridge lemmas for `parseAzPolynomial_toChars_nat` -/

-- Shorthand: the ordered nonzero (degree, coeff) pairs of p
private abbrev nzPairs (p : AzPolynomial ℕ) :=
...
private abbrev monoStrings (p : AzPolynomial ℕ) :=
...
lemma toChars_toList_nat_ne_zero (p : AzPolynomial ℕ) (hp : p ≠ 0) :
...
lemma splitPolynomialChars_toChars_nat (p : AzPolynomial ℕ) (hp : p ≠ 0) :
  ...
  · -- (monoStrings p).reverse ≠ [] because p ≠ 0 has at least one nonzero coefficient
    have h_back : p.coeffs.back? ≠ none := by
      intro h_none
      have hp2 : p.coeffs = #[] := Array.back?_eq_none_iff.mp h_none
      have hpe : p.coeffs = (0 : AzPolynomial ℕ).coeffs := by rw [coeffs_zero, hp2]
      exact hp (AzPolynomial.ext hpe)
    have h_back2 : ∃ c, p.coeffs.back? = some c := by
      rcases Option.ne_none_iff_exists.mp h_back with ⟨c, hc_eq⟩
      exact ⟨c, hc_eq.symm⟩
    rcases h_back2 with ⟨c, hc_eq⟩
...
lemma parsedParts_allSome_nat (p : AzPolynomial ℕ) (_ : p ≠ 0) :
...
/-! ### Bridge lemmas for `parseAzPolynomial_toChars_zmod` -/

-- Shorthand: the ordered nonzero (degree, coeff) pairs of a ZMod polynomial
private abbrev nzPairs_zmod {n : ℕ} [NeZero n] (p : AzPolynomial (ZMod n)) :=
  (listEnum p.coeffs.toList).filter (fun dc => decide (dc.2 ≠ 0))

-- Shorthand: their monomial string representations (low-to-high degree order)
private abbrev monoStrings_zmod {n : ℕ} [NeZero n] (p : AzPolynomial (ZMod n)) :=
...
lemma splitPolynomialChars_toChars_zmod {n : ℕ} [NeZero n]
  ...
  · have h_back : p.coeffs.back? ≠ none := by
      intro h_none
      have hp2 : p.coeffs = #[] := Array.back?_eq_none_iff.mp h_none
      have hpe : p.coeffs = (0 : AzPolynomial (ZMod n)).coeffs := by rw [coeffs_zero, hp2]
      exact hp (AzPolynomial.ext hpe)
...
theorem parseAzPolynomial_toChars_nat (p : AzPolynomial ℕ) :
    parseAzPolynomial (toChars p) = some p := by
  by_cases hp : p = 0
  · -- Case p = 0: toChars 0 = "0", parseAzPolynomial "0" = some 0 ✓
    subst hp
    simp [parseAzPolynomial, toChars]
  · -- Case p ≠ 0
    ...
    have hne : toChars p ≠ "0" := by
      ...
      exact hbad rfl
    -- Unfold parseAzPolynomial step by step
    simp only [parseAzPolynomial, if_neg hne]
...
theorem parseAzPolynomial_toChars_zmod {n : ℕ} [NeZero n] (p : AzPolynomial (ZMod n)) :
    parseAzPolynomial (toChars p) = some p := by
  by_cases hp : p = 0
  · subst hp; simp [parseAzPolynomial, toChars]
  · -- Show toChars p ≠ "0"
    have hne : toChars p ≠ "0" := by
      intro h
...
/-! ### Bridge lemmas for `parseAzPolynomial_toChars_int` -/

-- Shorthand: the ordered nonzero (degree, coeff) pairs of an ℤ polynomial
private abbrev nzPairs_int (p : AzPolynomial ℤ) :=
  (listEnum p.coeffs.toList).filter (fun dc => decide (dc.2 ≠ 0))

-- Shorthand: their monomial string representations (low-to-high degree order)
private abbrev monoStrings_int (p : AzPolynomial ℤ) :=
...
private lemma toChars_toList_int_ne_zero (p : AzPolynomial ℤ) (hp : p ≠ 0) :
...
lemma splitPolynomialChars_toChars_int (p : AzPolynomial ℤ) (hp : p ≠ 0) :
...
lemma parsedParts_allSome_int (p : AzPolynomial ℤ) (_ : p ≠ 0) :
...
theorem parseAzPolynomial_toChars_int (p : AzPolynomial ℤ) :
    parseAzPolynomial (toChars p) = some p := by
  by_cases hp : p = 0
  · subst hp; simp [parseAzPolynomial, toChars]
  · have hne : toChars p ≠ "0" := by
...
/-! ### Bridge lemmas for `parseAzPolynomial_toChars_rat` -/
...
private abbrev nzPairs_rat (p : AzPolynomial ℚ) :=
  (listEnum p.coeffs.toList).filter (fun dc => decide (dc.2 ≠ 0))

-- Shorthand: their monomial string representations (low-to-high degree order)
private abbrev monoStrings_rat (p : AzPolynomial ℚ) :=
...
private lemma toChars_toList_rat_ne_zero (p : AzPolynomial ℚ) (hp : p ≠ 0) :
...
lemma splitPolynomialChars_toChars_rat (p : AzPolynomial ℚ) (hp : p ≠ 0) :
...
lemma parsedParts_allSome_rat (p : AzPolynomial ℚ) (_ : p ≠ 0) :
...
theorem parseAzPolynomial_toChars_rat (p : AzPolynomial ℚ) :
    parseAzPolynomial (toChars p) = some p := by
  by_cases hp : p = 0
  · subst hp; simp [parseAzPolynomial, toChars]
  · have hne : toChars p ≠ "0" := by
