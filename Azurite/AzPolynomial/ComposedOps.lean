...
-- 2 + 3 = 5
#guard toChars (composedSum (pp "x-2") (pp "x-3")) == "x-5"

-- √2 + √3: the classic x⁴ − 10x² + 1
#guard toChars (composedSum (pp "x^2-2") (pp "x^2-3")) == "x^4-10*x^2+1"

-- ±√2 + 1: x² − 2x − 1
#guard toChars (composedSum (pp "x^2-2") (pp "x-1")) == "x^2-2*x-1"

-- golden ratio pairs: φ+φ = 1+√5, φ+ψ = 1 (twice), ψ+ψ = 1−√5:
-- (x − 1)² (x² − 2x − 4)
#guard toChars (composedSum (pp "x^2-x-1") (pp "x^2-x-1")) == "x^4-4*x^3+x^2+6*x-4"

-- ±i + ±i: {2i, 0, 0, −2i}: x² (x² + 4)
#guard toChars (composedSum (pp "x^2+1") (pp "x^2+1")) == "x^4+4*x^2"

-- 2 · 3 = 6
#guard toChars (composedProduct (pp "x-2") (pp "x-3")) == "x-6"

-- √2 · √3 pairs: ±√6 twice: (x² − 6)²
#guard toChars (composedProduct (pp "x^2-2") (pp "x^2-3")) == "x^4-12*x^2+36"

-- ±√2 · 3: x² − 18
#guard toChars (composedProduct (pp "x^2-2") (pp "x-3")) == "x^2-18"

-- ±i · ±i: {−1, 1, 1, −1}: (x² − 1)²
#guard toChars (composedProduct (pp "x^2+1") (pp "x^2+1")) == "x^4-2*x^2+1"

-- Q with a root at 0: x(x − 1) — the 0 root is dropped, 2·1 = 2 survives
#guard toChars (composedProduct (pp "x-2") (pp "x^2-x")) == "x-2"

-- 5 − 3 = 2 (sign of the output is not normalized)
#guard toChars (composedDifference (pp "x-5") (pp "x-3")) == "-x+2"

-- ±√2 ∓ √3: same quartic as the sum, by symmetry
#guard toChars (composedDifference (pp "x^2-2") (pp "x^2-3")) == "x^4-10*x^2+1"

-- self-difference: {0, 0, ±2√2}: x²(x²−8)
#guard toChars (composedDifference (pp "x^2-2") (pp "x^2-2")) == "x^4-8*x^2"

-- φ − 1 = 1/φ and ψ − 1: x² + x − 1
#guard toChars (composedDifference (pp "x^2-x-1") (pp "x-1")) == "x^2+x-1"

-- fusion agrees with the composition
#guard toChars (composedDifference (pp "x^2-x-1") (pp "x^2-3"))
    == toChars (composedSum (pp "x^2-x-1") ((pp "x^2-3").negateRoots))

-- 6 / 3 = 2 (output scaled by coefficients of Q)
#guard toChars (composedQuotient (pp "x-6") (pp "x-3")) == "-3*x+6"

-- ±3√2 / 3 = ±√2: 9(x² − 2)
#guard toChars (composedQuotient (pp "x^2-18") (pp "x-3")) == "9*x^2-18"

-- ±√2 / ±√3 = ±√(2/3), each twice: (3x² − 2)²
#guard toChars (composedQuotient (pp "x^2-2") (pp "x^2-3")) == "9*x^4-12*x^2+4"

-- self-quotient: {1, 1, −1, −1}: 4(x² − 1)²
#guard toChars (composedQuotient (pp "x^2-2") (pp "x^2-2")) == "4*x^4-8*x^2+4"

-- Q with a root at 0 but P(0) ≠ 0: the 0 root contributes nothing
#guard toChars (composedQuotient (pp "x-2") (pp "x^2-x")) == "-2*x+4"

-- indeterminate 0 / 0: both vanish at 0, so the resultant is identically zero
#guard toChars (composedQuotient (pp "x^2-x") (pp "x^2-x")) == "0"

-- fusion agrees with the composition
#guard toChars (composedQuotient (pp "x^2-x-1") (pp "x^2-3"))
    == toChars (composedProduct (pp "x^2-x-1") ((pp "x^2-3").invertRoots))

-- eliminateCoeffs: rational coefficients recover the polynomial (up to sign):
-- F = x − 2 via c₀ = −2, c₁ = 1
#guard toChars (eliminateCoeffs [pp "x+2", pp "x-1"]) == "-x+2"

-- F = x ∓ √2: both conjugate choices: x² − 2
#guard toChars (eliminateCoeffs [pp "x^2-2", pp "x-1"]) == "x^2-2"

-- F = x² ∓ √2: the fourth root of 2: (x²−√2)(x²+√2) = x⁴ − 2
#guard toChars (eliminateCoeffs [pp "x^2-2", pp "x", pp "x-1"]) == "x^4-2"

-- F = ±√2·x ± √2: roots ∓1 over the four sign choices: 4(x²−1)²
#guard toChars (eliminateCoeffs [pp "x^2-2", pp "x^2-2"]) == "4*x^4-8*x^2+4"

-- F = x + c₀ with c₀² + c₀ − 1 = 0: roots are the golden-ratio pair
#guard toChars (eliminateCoeffs [pp "x^2+x-1", pp "x-1"]) == "x^2-x-1"

-- two algebraic coefficients: F = x² ± √2·x ± √3, all four conjugates (degree 8)
#guard toChars (eliminateCoeffs [pp "x^2-3", pp "x^2-2", pp "x-1"])
    == "x^8-4*x^6-2*x^4-12*x^2+9"

-- degenerate: c₀ may be 0 and F is the constant c₀ — every z must be covered
#guard toChars (eliminateCoeffs [pp "x"]) == "0"

-- nonzero constant F = −5: no roots, and the output is a root-free constant
#guard toChars (eliminateCoeffs [pp "x-5"]) == "-5"

-- empty list: F = 0 identically
#guard toChars (eliminateCoeffs ([] : List (AzPolynomial AzInt))) == "0"
