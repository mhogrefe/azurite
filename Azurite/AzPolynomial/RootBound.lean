...
/-!
# Computable Root Bound for AzPolynomial
...
def maxCoeffRatio (f : R →+* ℚ) (coeffs : Array R) : ℚ :=
  if h : coeffs.size ≤ 1 then 0
    ...
    if lead == 0 then 0  -- shouldn't happen for well-formed AzPolynomial
    else
      let n := coeffs.size - 1
...
def rootBound (f : R →+* ℚ) (p : AzPolynomial R) : ℚ :=
...
/-- Root bound for `AzPolynomial ℤ` using the canonical embedding `ℤ ↪ ℚ`. -/
def rootBoundInt (p : AzPolynomial ℤ) : ℚ :=
  rootBound (Int.castRingHom ℚ) p

/-- Root bound for `AzPolynomial ℚ`. -/
def rootBoundRat (p : AzPolynomial ℚ) : ℚ :=
