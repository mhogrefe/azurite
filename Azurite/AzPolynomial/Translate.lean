...
#guard (parseAzPolynomial (R := ℤ) "x^2+1").get!.translate 1
    == (parseAzPolynomial (R := ℤ) "x^2-2*x+2").get!

-- x³ translated by -1: (x+1)³ = x³ + 3x² + 3x + 1
#guard (parseAzPolynomial (R := ℤ) "x^3").get!.translate (-1)
    == (parseAzPolynomial (R := ℤ) "x^3+3*x^2+3*x+1").get!

-- Constant translated by anything: unchanged
#guard (parseAzPolynomial (R := ℤ) "5").get!.translate 42
    == (parseAzPolynomial (R := ℤ) "5").get!

-- Zero translated: zero
#guard (0 : AzPolynomial ℤ).translate 7 == 0

-- 2x + 3 translated by 5: 2(x-5) + 3 = 2x - 7
#guard (parseAzPolynomial (R := ℤ) "2*x+3").get!.translate 5
    == (parseAzPolynomial (R := ℤ) "2*x-7").get!
