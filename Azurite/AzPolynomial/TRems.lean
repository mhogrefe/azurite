...
* `tremsLeaves P Q` — returns only the leaf polynomials, which is the
...
/-- Shared termination argument for `mkTRemsNode` and `mkTRemsLeavesAux`. -/
...
/-- Collect only the **leaf polynomials** of `TRems(P, Q)`.
More efficient than building the full tree when only the leaves
are needed. A leaf is any node with no children — either because
`Pol(N) = 0` or because the pseudo-remainder sequence terminated. -/
def mkTRemsLeavesAux
    (parentPol curPol : AzPolynomial (AzMvPolynomial k D ord)) :
  ...
  if _h : curPol == 0 then [curPol]
    ...
    | _ => children.attach.flatMap fun ⟨child, _hmem⟩ =>
        mkTRemsLeavesAux curPol child
termination_by curPol.natDegree
decreasing_by
  simp only [beq_iff_eq] at _h
  exact natDegree_child_lt_of_mem_tru_neg_pRem _h _hmem

/-- The **leaf polynomials** of `TRems(P, Q)`. -/
def tremsLeaves
    (P Q : AzPolynomial (AzMvPolynomial k D ord)) :
    List (AzPolynomial (AzMvPolynomial k D ord)) :=
  (tru Q).flatMap (mkTRemsLeavesAux P)
...
-- Q = 0: tru(0) = [], so tremsLeaves has no branches
#guard tremsLeaves (p "x+(a)") (p "0") == []
...
#guard (tremsLeaves (p "x+(a)") (p "(1)")).map s == ["(1)"]
...
#guard (tremsLeaves (p "x^2+(b)") (p "x+(a)")).length == 1
...
-- All four leaves, collected by tremsLeaves
#guard (tremsLeaves (p "x^4+(a)*x^2+(b)*x+(c)")
