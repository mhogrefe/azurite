...
/-!
# DensePolyQ: Rational Polynomials with Shared Denominator

This module defines `DensePolyQ`, a canonical dense representation of univariate
polynomials over `ℚ`. A `DensePolyQ` stores:
...
namespace DensePolyQ

/-! ## Basic instances and zero -/

/-- Extensionality: two `DensePolyQ` are equal iff their numerators and
    denominator are equal. -/
@[ext]
lemma ext {p q : DensePolyQ}
...
instance : DecidableEq DensePolyQ := fun p q =>
  if h1 : p.numerators = q.numerators then
    if h2 : p.denom = q.denom then
      isTrue (DensePolyQ.ext h1 h2)
    else
      isFalse (fun h => h2 (congrArg DensePolyQ.denom h))
  else
    isFalse (fun h => h1 (congrArg DensePolyQ.numerators h))
...
instance {p : DensePolyQ} : Decidable p.Monic := by
