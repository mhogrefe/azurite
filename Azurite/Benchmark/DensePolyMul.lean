...
/-- Sum of (log2 + 1) over all coefficients — matches the Rust DensePoly::significant_bits. -/
def densePolyNatSignificantBits (p : DensePoly ℕ) : Nat :=
  p.coeffs.foldl (fun acc c => acc + if c = 0 then 0 else c.log2 + 1) 0

-- ── Benchmark ───────────────────────────────────────────────────────────────

/--
Run the `dense_poly_mul` benchmark.
For each of `limit` pairs `(a, b)` of random `DensePoly ℕ`, time:
...
-/
def runDensePolyMul (limit : Nat) (cfg : Std.HashMap String String) (seed : UInt64) : IO Unit := do
  ...
  let mut g := mkDensePolyNatRandomGen meanDegree meanCoeffBitLength seed
  for _ in List.range limit do
    let (a, g') := DensePolyRandomGen.next g
    let (b, g'') := DensePolyRandomGen.next g'
    let sb := densePolyNatSignificantBits a + densePolyNatSignificantBits b
    -- mulBasecaseList
