...
distribution of mean `meanBitLength`), time `a * a` two ways:
  ...
  - `square a`           — uses `schoolbookSquareLimbs` (n(n+1)/2 wideMuls)

Output format:
  `<sb>;mulSchoolbook,<ns>;square,<ns>`
...
def runAzNatSquare (limit : Nat) (cfg : Std.HashMap String String) (seed : UInt64) : IO Unit := do
  ...
  for _ in List.range limit do
    ...
    let (rSq, ns2a) ← timeNsIter iters (fun _ => Azurite.AzNat.square azA)
    ...
    if Azurite.AzNat.toNat rSq ≠ Azurite.AzNat.toNat rMul then
      IO.eprintln s!"BUG: square disagrees with mulSchoolbook for a={a}"
    IO.println s!"{sb};mulSchoolbook,{ns1};square,{ns2}"
    g := g'
