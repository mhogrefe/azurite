...
private def cmpM (a b : MonicMonomial n ord) : Ordering :=
  compareExponents ord a.exponents b.exponents

private theorem cmpM_refl (a : MonicMonomial n ord) : cmpM a a = .eq :=
  compareExponents_refl ord _

private theorem cmpM_swap (a b : MonicMonomial n ord) :
    (cmpM a b).swap = cmpM b a :=
  compareExponents_swap ord _ _

private theorem cmpM_eq (a b : MonicMonomial n ord) (h : cmpM a b = .eq) : a = b :=
  MonicMonomial.ext (compareExponents_eq ord _ _ h)

private theorem cmpM_trans_lt (a b c : MonicMonomial n ord)
    (h1 : cmpM a b = .lt) (h2 : cmpM b c = .lt) : cmpM a c = .lt :=
  compareExponents_trans_lt ord _ _ _ h1 h2

private theorem cmpM_lt_of_gt {a b : MonicMonomial n ord} (h : cmpM a b = .gt) :
    cmpM b a = .lt := by
  have := cmpM_swap a b; rw [h] at this; exact this.symm

private theorem cmpM_gt_of_lt {a b : MonicMonomial n ord} (h : cmpM a b = .lt) :
    cmpM b a = .gt := by
  have := cmpM_swap a b; rw [h] at this; exact this.symm

instance : LE (MonicMonomial n ord) := ⟨fun a b => cmpM a b ≠ .gt⟩
instance : LT (MonicMonomial n ord) := ⟨fun a b => cmpM a b = .lt⟩

instance : DecidableRel (LE.le (α := MonicMonomial n ord)) := fun a b =>
  inferInstanceAs (Decidable (cmpM a b ≠ .gt))

instance : DecidableRel (LT.lt (α := MonicMonomial n ord)) := fun a b =>
  inferInstanceAs (Decidable (cmpM a b = .lt))

instance : PartialOrder (MonicMonomial n ord) where
  le_refl a := by
...
instance instLinearOrder : LinearOrder (MonicMonomial n ord) where
  le_total a b := by
