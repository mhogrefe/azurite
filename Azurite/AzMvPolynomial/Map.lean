/-
  Map functions for AzMvPolynomial.
  Analogous to the map functions in AzPolynomial.Basic.
-/
...
def mapMonomialCoeff (f : R →+* S)
    (m : Monomial σ R ord) : Option (Monomial σ S ord) :=
...
theorem mapMonomialCoeff_some {f : R →+* S}
    {m : Monomial σ R ord} {m' : Monomial σ S ord}
...
def mapCoeff (f : R → S) (hf : ∀ r, f r = 0 → r = 0)
    (m : Monomial σ R ord) : Monomial σ S ord :=
...
/-- Map coefficients via an injective ring homomorphism.
    No zero-filtering needed since injectivity implies `f r = 0 → r = 0`. -/
...
/-- Map coefficients via a ring homomorphism, filtering out zero results.
    This is the most general map: `f` may send nonzero coefficients to zero.
    Monic parts are unchanged, so sorted order is preserved. -/
def AzMvPolynomial.map
    (f : R →+* S) (p : AzMvPolynomial σ R ord) : AzMvPolynomial σ S ord :=
