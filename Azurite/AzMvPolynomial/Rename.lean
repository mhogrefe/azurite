...
/-
  Renaming variables for `AzMvPolynomial` — Fin-indexed core.
...
open MonicMonomial Monomial MonomialOrder AzMvPolynomial
...
def collapseAux (cur : MonicMonomial n ord × R)
    (acc : List (MonicMonomial n ord × R))
    (rest : List (Monomial n R ord)) : List (MonicMonomial n ord × R) :=
  match rest with
  | [] => (cur :: acc).reverse
  | m :: rest' =>
...
def collapseMonics (l : List (Monomial n R ord)) :
    List (MonicMonomial n ord × R) :=
  match l with
  | [] => []
...
theorem pairwise_filterMap_gt [DecidableEq R]
    {l : List (MonicMonomial n ord × R)}
    (hp : l.Pairwise (fun a b => a.1 > b.1)) :
    (l.filterMap (fun p =>
      if h : p.2 = 0 then none else some ⟨⟨p.2, h⟩, p.1⟩)).Pairwise
      (fun (a : Monomial n R ord) (b : Monomial n R ord) => a.monic > b.monic) :=
...
def AzMvPolynomial.rename [DecidableEq R]
    (p : AzMvPolynomial n₁ R ord) (f : Fin n₁ → Fin n₂)
    (ord₂ : MonomialOrder := ord) : AzMvPolynomial n₂ R ord₂ :=
...
/-- `MonicMonomial.rename` is injective when `f : Fin n₁ → Fin n₂` is
...
theorem MonicMonomial.rename_injective
    ...
    Function.Injective (fun m : MonicMonomial n₁ ord => m.rename f ord₂) := by
  intro a b hab
  apply MonicMonomial.ext
  apply Vector.ext
  intro idx hidx
  let i : Fin n₁ := ⟨idx, hidx⟩
  have h := congrArg (fun m : MonicMonomial n₂ ord₂ => m.exponents[(f i).val]'(f i).isLt) hab
  simp only [MonicMonomial.rename, Vector.getElem_ofFn] at h
  have hfilter : ∀ v : MonicMonomial n₁ ord,
...
def AzMvPolynomial.renameInjective
  ...
  ⟨sorted.toArray, by
    ...
    have hrenamed_monic_nodup : (renamed.map Monomial.monic).Nodup := by
      ...
      rw [show (fun m : Monomial n₁ R ord => m.monic.rename f ord₂) =
...
def AzMvPolynomial.renameMonotone
    (p : AzMvPolynomial n₁ R ord) (f : Fin n₁ → Fin n₂)
    (hg : StrictMono f) : AzMvPolynomial n₂ R ord :=
  let renamed := p.terms.toList.map (fun m => m.rename f ord)
  ⟨renamed.toArray, by
    rw [List.toList_toArray]
...
def AzMvPolynomial.embed
    (h : n₁ ≤ n₂) (p : AzMvPolynomial n₁ R ord) : AzMvPolynomial n₂ R ord :=
