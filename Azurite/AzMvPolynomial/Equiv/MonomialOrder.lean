...
Constructs a Mathlib `MonomialOrder σ` from our ordering schemes and proves
...
/-! ### Auxiliary lemmas on Array.foldl -/
...
theorem vec_entry_le_totalDeg (v : Vector ℕ n) (i : Fin n) :
    v[i] ≤ totalDeg v := by
...
theorem vec_zero_of_totalDeg_zero (v : Vector ℕ n) (h : totalDeg v = 0) (i : Fin n) :
    v[i] = 0 := by
...
theorem compareExponents_ge_zero (ord : MonomialOrder) (v : Vector ℕ n) :
...
theorem MonicMonomial.one_le' (m : MonicMonomial σ ord) : 1 ≤ m := by
  rw [not_lt.symm]
  intro (h : compareExponents ord m.exponents (1 : MonicMonomial σ ord).exponents = .lt)
  have h1 : (1 : MonicMonomial σ ord).exponents = Vector.ofFn (fun _ => 0) := by
    ext i hi; simp [one_exponents]
  rw [h1] at h
  exact compareExponents_ge_zero ord m.exponents h

theorem MonicMonomial.mul_le_mul_left' (c : MonicMonomial σ ord)
    {a b : MonicMonomial σ ord} (h : a ≤ b) : c * a ≤ c * b := by
...
section Monotone
variable {σ : Type _} {n : ℕ} [LinearOrder σ] [Var σ n] [DecidableEq σ]
         {ord : MonomialOrder}

theorem ofFinsupp_monotone (f g : σ →₀ ℕ) (h : f ≤ g) :
  ...
  calc MonicMonomial.ofFinsupp (ord := ord) f
    ...
    _ ≤ MonicMonomial.ofFinsupp f * MonicMonomial.ofFinsupp (g - f) := by
        rcases lt_or_eq_of_le (MonicMonomial.one_le' (MonicMonomial.ofFinsupp (g - f))) with hlt | heq
...
instance wellFoundedLT_monicMonomial : WellFoundedLT (MonicMonomial σ ord) where
  wf := by
    rw [wellFounded_iff_isEmpty_descending_chain]
    by_contra h; rw [not_isEmpty_iff] at h
    obtain ⟨f, hf⟩ := h
    have hpwo := Set.isPWO_of_wellQuasiOrderedLE (Set.univ : Set (σ →₀ ℕ))
...
/-! ### Constructing a Mathlib MonomialOrder -/

section Bridge
variable {σ : Type _} {n : ℕ} [LinearOrder σ] [Var σ n] [DecidableEq σ]

/-- Construct a Mathlib `MonomialOrder σ` from our ordering. -/
noncomputable def toMathlibMonomialOrder (ord : MonomialOrder) :
    _root_.MonomialOrder σ where
  syn := Additive (MonicMonomial σ ord)
  ...
  iocam := {
    add_le_add_left := fun a b hab c => by
      ...
      rw [MonicMonomial.mul_comm (Additive.toMul a), MonicMonomial.mul_comm (Additive.toMul b)]
...
variable {R : Type _} [Field R]
         {σ : Type _} {n : ℕ} [LinearOrder σ] [Var σ n] [DecidableEq σ]
         {ord : MonomialOrder}

private theorem toSyn_toFinsupp (m : MonomialOrder) (a : MonicMonomial σ m) :
    (toMathlibMonomialOrder (σ := σ) (n := n) m).toSyn a.toFinsupp = Additive.ofMul a :=
  congrArg Additive.ofMul (MonicMonomial.ofFinsupp_toFinsupp a)

omit [DecidableEq σ] in
private theorem terms_monic_le_zero [DecidableEq R] (p : AzMvPolynomial σ R ord)
...
theorem terms_zero_mem_support [DecidableEq R] (p : AzMvPolynomial σ R ord)
    (hp : p.terms.size > 0) :
...
theorem degree_eq_terms_zero [DecidableEq R] (p : AzMvPolynomial σ R ord)
    (hp : p.terms.size > 0) :
    (toMathlibMonomialOrder (σ := σ) (n := n) ord).degree p.toMvPoly =
...
theorem leadingCoeff_eq_terms_zero [DecidableEq R] (p : AzMvPolynomial σ R ord)
    (hp : p.terms.size > 0) :
    (toMathlibMonomialOrder (σ := σ) (n := n) ord).leadingCoeff p.toMvPoly =
