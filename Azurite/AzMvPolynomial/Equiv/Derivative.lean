...
private theorem toMvPoly_pderivAt_eq (m : Monomial n R ord) (j : Fin n) :
    ((m.pderivAt j).elim 0 Monomial.toMvPoly) =
...
theorem toMvPoly_pderivGeneral (j : Fin n) (p : AzMvPolynomial n R ord) :
    (AzMvPolynomial.pderivGeneral j p).toMvPoly =
  ...
  conv_lhs => rw [show p.terms.toList.map (fun x =>
    (x.pderivAt j).elim 0 Monomial.toMvPoly) =
    p.terms.toList.map (fun m => (MvPolynomial.pderiv j) m.toMvPoly)
    from List.map_congr_left fun m _ => toMvPoly_pderivAt_eq m j]
  rw [show (fun m : Monomial n R ord => (MvPolynomial.pderiv j) m.toMvPoly) =
    (⇑(MvPolynomial.pderiv j)) ∘ Monomial.toMvPoly from rfl,
