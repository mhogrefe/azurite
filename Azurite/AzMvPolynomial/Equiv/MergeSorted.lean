/-
  ...
  the sum of `toMvPoly` over the merged result equals the sum over the
  first list, plus the sum over the second list with `f` applied to
  each coefficient.
-/
...
/-- `mergeSorted f hf` preserves the MvPolynomial sum:
    the result sums to `sum(ps.map toMvPoly) + sum(qs.map (monomial · ∘ f ·))`. -/
  ...
  | p :: ps', q :: qs' =>
    ...
    · -- equal monics, nonzero sum: emit combined monomial
      ...
      have heq : p.monic = q.monic :=
        le_antisymm (not_lt.mp hpq) (not_lt.mp hqp)
      have hcomb :
          (⟨⟨p.coeff.val + f q.coeff.val, hc⟩, p.monic⟩ : Monomial σ R ord).toMvPoly =
