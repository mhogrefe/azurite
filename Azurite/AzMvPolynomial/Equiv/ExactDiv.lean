...
theorem toMvPoly_ofMonomial (m : Monomial n R ord) :
...
theorem toMvPoly_monomialMul (m : Monomial n R ord) (p : AzMvPolynomial n R ord) :
...
theorem support_card_le_pow (c : MvPolynomial (Fin n) R) (d : ℕ)
    (hd : ∀ m ∈ c.support, ∀ v : Fin n, m v ≤ d) :
...
theorem quotient_totalDegree_le (c q : MvPolynomial (Fin n) R) (hc : c ≠ 0) (hq : q ≠ 0) :
...
private theorem exactDivStep_quotient_support_shrinks
  ...
  have hqnz : q.toMvPoly ≠ 0 := fun h => absurd (terms_empty_of_toMvPoly_eq_zero q
    (by have := numTerms_eq_support_card q
        rw [AzMvPolynomial.numTerms, h, support_zero, Finset.card_empty] at this;
        omega)) (by omega)
...
private theorem remainder_zero_of_support_card
  ...
  | succ fuel ih =>
    ...
    · next hr' =>
      ...
      obtain ⟨c', hcq', hlt⟩ :=
        exactDivStep_quotient_support_shrinks r q hr' hq c hcq hc
...
theorem remainder_zero_of_dvd (p q : AzMvPolynomial n R ord) (hq : q.terms.size > 0)
...
theorem exactDiv_spec (p q : AzMvPolynomial n R ord) (hq : q.terms.size > 0)
    (hdvd : q.toMvPoly ∣ p.toMvPoly) :
...
theorem exactDiv_mul_eq (p q : AzMvPolynomial n R ord) (hq : q.terms.size > 0)
    (hdvd : q.toMvPoly ∣ p.toMvPoly) :
...
theorem ofMvPoly_exactDiv_mul (p' q' : MvPolynomial (Fin n) R) (hdvd : q' ∣ p')
    (hq : (AzMvPolynomial.ofMvPoly q' : AzMvPolynomial n R ord).terms.size > 0) :
  ...
  have hdvd' : (AzMvPolynomial.ofMvPoly q' : AzMvPolynomial n R ord).toMvPoly ∣
               (AzMvPolynomial.ofMvPoly p' : AzMvPolynomial n R ord).toMvPoly := by
