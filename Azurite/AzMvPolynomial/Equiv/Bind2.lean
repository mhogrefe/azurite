...
theorem toMvPoly_monicToAzMvPoly (m : MonicMonomial σ ord) :
    toMvPoly (m.toAzMvPoly : AzMvPolynomial σ S ord) =
...
theorem toMvPoly_monomial_bind₂ (f : R → AzMvPolynomial σ S ord)
    (m : Monomial σ R ord) :
    toMvPoly (m.bind₂ f) =
...
private theorem toMvPoly_foldl_bind₂
    (l : List (Monomial σ R ord)) (f : R → AzMvPolynomial σ S ord)
    (acc : AzMvPolynomial σ S ord) :
...
@[simp] theorem toMvPoly_bind₂ (f : R →+* MvPolynomial σ S)
  ...
  have hzero : toMvPoly (0 : AzMvPolynomial σ S ord) = (0 : MvPolynomial σ S) := toMvPoly_zero
  rw [hzero, zero_add]
...
omit [NoZeroDivisors R] [DecidableEq R] in
@[simp] theorem ofMvPoly_bind₂ (f : R →+* MvPolynomial σ S)
    (p : MvPolynomial σ R) :
