...
For `[Unique σ]` (single-variable multivariate polynomials), we prove:

  `AzPolynomial.toPoly (p.toAzPolynomial) = MvPolynomial.eval₂ Polynomial.C (fun _ => Polynomial.X) (toMvPoly p)`

This corresponds to the `MvPolynomial.pUnitAlgEquiv` direction in Mathlib.
-/
...
/-! ### Finsupp representation for Unique σ -/

omit [CommSemiring R] [DecidableEq R] in
/-- For `[Unique σ]`, a monic monomial's finsupp is `single default degree`. -/
theorem MonicMonomial.toFinsupp_eq_single_degree [DecidableEq σ] [Unique σ]
    (m : MonicMonomial σ ord) :
    m.toFinsupp = Finsupp.single default m.degree := by
  ext
  simp only [MonicMonomial.toFinsupp, Finsupp.onFinset_apply, Finsupp.single_apply,
    MonicMonomial.degree, MonicMonomial.exponent, ite_true]

/-! ### eval₂ of a single monomial -/

omit [DecidableEq R] in
/-- `eval₂ C (fun _ => X) (monomial s c) = Polynomial.monomial (degree) c` for Unique σ. -/
theorem eval₂_monomial_unique [Unique σ]
    (m : MonicMonomial σ ord) (c : R) :
...
omit [DecidableEq R] in
private theorem eval₂_sum_map_toMvPoly [Unique σ]
...
omit [DecidableEq R] in
private theorem foldl_set_size_list [Unique σ]
    (l : List (Monomial σ R ord)) (acc : Array R) :
...
omit [DecidableEq R] in
private theorem foldl_set_getElem [Unique σ]
    (l : List (Monomial σ R ord))
...
/-! ### Ordering equivalence for Unique σ -/

omit [CommSemiring R] [DecidableEq R] in
/-- For `[Unique σ]`, the linear order on `MonicMonomial` agrees with the ℕ order on degree. -/
theorem MonicMonomial.lt_iff_degree_lt [Unique σ]
    (m₁ m₂ : MonicMonomial σ ord) : m₁ < m₂ ↔ m₁.degree < m₂.degree := by
  have hn1 := Var.n_eq_one (σ := σ)
  ...
  have revlex_one : ∀ (a b : Vector ℕ 1), revlexCompareAux a b 0 = (compare a[0] b[0]).swap := by
    ...
    · rename_i heq; show revlexCompareAux a b 1 = _
      unfold revlexCompareAux; simp only [show ¬ (1 < 1) from lt_irrefl _, ↓reduceDIte]; rw [heq]; rfl
  ...
  have hfin : Var.toFin (default : σ) = (⟨0, by omega⟩ : Fin 1) := by
    ext; have := (Var.toFin (default : σ)).isLt; omega
  simp only [hfin]; exact Nat.compare_eq_lt

/-! ### Distinct degrees from AzMvPolynomial invariant -/

omit [DecidableEq R] in
private theorem pairwise_degree_ne_of_sorted [Unique σ]
...
private theorem degrees_le_lead [Unique σ]
    (terms : Array (Monomial σ R ord))
    (hsorted : terms.toList.Pairwise (fun a b => a.monic > b.monic))
    (hne : terms.size ≠ 0) :
...
/-- Converting an `AzMvPolynomial` with `[Unique σ]` to an `AzPolynomial` and then to
    Mathlib's `Polynomial` agrees with applying `eval₂ C X` to the `MvPolynomial` form. -/
theorem toPoly_toAzPolynomial [Unique σ]
  ...
  · unfold AzMvPolynomial.toAzPolynomial; rw [dif_neg h]; simp only; rw [toPoly_normalize]
    ...
    -- RHS: distribute coeff, simplify via coeff_monomial, then use sum_ite_eq_find
    have hcoeff_sum :
        (p.terms.toList.map (fun m => Polynomial.monomial m.monic.degree m.coeff.val)).sum.coeff i =
    ...
    -- LHS: unfold to array access, convert Array.foldl to List.foldl
    ...
    have hsize : (Array.replicate ((p.terms[0]'(by omega)).monic.degree + 1) (0 : R)).size =
    ...
    · -- In-bounds: use foldl_set_getElem
      ...
      have hi_acc : i < (Array.replicate ((p.terms[0]'(by omega)).monic.degree + 1) (0 : R)).size :=
        hsize.symm ▸ hi
      have hdegs_acc : ∀ m ∈ p.terms.toList,
          m.monic.degree < (Array.replicate ((p.terms[0]'(by omega)).monic.degree + 1) (0 : R)).size :=
    ...
    · -- Out-of-bounds: both sides are 0
