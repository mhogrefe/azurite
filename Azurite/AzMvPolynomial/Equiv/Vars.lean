...
theorem MonicMonomial.vars_eq_toFinsupp_support (m : MonicMonomial σ ord) :
    m.vars = m.toFinsupp.support := by
  ext v
  simp only [MonicMonomial.vars, Finset.mem_image, Finset.mem_filter, Finset.mem_univ, true_and,
    Finsupp.mem_support_iff, MonicMonomial.toFinsupp, Finsupp.onFinset_apply]
  constructor
  · rintro ⟨i, hi, rfl⟩
    simp only [Var.toFin_ofFin]; exact hi
  · intro hv
    exact ⟨Var.toFin v, hv, Var.ofFin_toFin v⟩
...
private theorem mem_foldl_union_vars
    (l : List (Monomial σ R ord)) (init : Finset σ) (v : σ) :
...
omit [NoZeroDivisors R] [DecidableEq R] in
@[simp] theorem toMvPoly_vars (p : AzMvPolynomial σ R ord) :
