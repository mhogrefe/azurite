...
/-!
# Equivalence: `AzMvPolynomialNew.toAzPolynomial` ↔ Mathlib
...
theorem MonicMonomialNew.toFinsupp_eq_single_degree
    (m : MonicMonomialNew 1 ord) :
...
private theorem eval₂_sum_map_toMvPoly_new
  ...
  | cons t ts ih =>
    ...
    congr 1
    exact eval₂_monomial_unique_new t.monic t.coeff.val

/-! ### Properties of the coefficient fold -/

omit [DecidableEq R] in
private theorem foldl_set_size_list_new
    (l : List (MonomialNew 1 R ord)) (acc : Array R) :
...
omit [DecidableEq R] in
private theorem foldl_set_getElem_new
    (l : List (MonomialNew 1 R ord))
    ...
    result[i]'(by rw [foldl_set_size_list_new]; exact hi) =
...
theorem sum_ite_eq_find_new {α R : Type _} [AddCommMonoid R]
...
omit [CommSemiring R] [DecidableEq R] in
/-- For `n = 1`, the linear order on `MonicMonomialNew` agrees with the ℕ order
    on `degree`. -/
theorem MonicMonomialNew.lt_iff_degree_lt
    (m₁ m₂ : MonicMonomialNew 1 ord) : m₁ < m₂ ↔ m₁.degree < m₂.degree := by
  ...
  have cmp_one : compareExponents ord m₁.exponents m₂.exponents =
    ...
    | Degrevlex =>
      ...
      · rfl
  rw [cmp_one]
  simp only [MonicMonomialNew.degree, MonicMonomialNew.exponent]
  exact Nat.compare_eq_lt

/-! ### Distinct degrees from sorted invariant -/

omit [DecidableEq R] in
private theorem pairwise_degree_ne_of_sorted_new
    (terms : Array (MonomialNew 1 R ord))
  ...
  hsorted.imp fun {a b} hab => by
    have := (MonicMonomialNew.lt_iff_degree_lt b.monic a.monic).mp hab; omega

private theorem degrees_le_lead_new
    (terms : Array (MonomialNew 1 R ord))
    (hsorted : terms.toList.Pairwise (fun a b => a.monic > b.monic))
    (hne : terms.size ≠ 0) :
  ...
  · obtain ⟨j, hj, rfl⟩ := List.getElem_of_mem hm
    ...
    · have hlt : 0 < j := Nat.pos_of_ne_zero hj0
      ...
      rw [h0] at this
      have := (MonicMonomialNew.lt_iff_degree_lt _ _).mp this; omega

/-! ### Main equivalence theorem -/

/-- Converting an `AzMvPolynomialNew 1 R ord` to an `AzPolynomial` and then to
    Mathlib's `Polynomial` agrees with applying `eval₂ C X` to the `MvPolynomial` form. -/
theorem toPoly_toAzPolynomial_new
    (p : AzMvPolynomialNew 1 R ord) :
    AzPolynomial.toPoly (p.toAzPolynomial) =
    MvPolynomial.eval₂ Polynomial.C (fun _ => Polynomial.X) (toMvPoly p) := by
  rw [AzMvPolynomialNew.toMvPoly_eq_list_sum, eval₂_sum_map_toMvPoly_new]
  by_cases h : p.terms.size = 0
  · have hterms : p.terms = #[] := Array.eq_empty_of_size_eq_zero h
    simp [AzMvPolynomialNew.toAzPolynomial, AzPolynomial.toPoly, AzPolynomial.zero,
      List.toPoly, hterms]
  · unfold AzMvPolynomialNew.toAzPolynomial; rw [dif_neg h]; simp only; rw [toPoly_normalize]
    ...
    rw [sum_ite_eq_find_new p.terms.toList
      ...
      (pairwise_degree_ne_of_sorted_new p.terms p.sorted) i]
    simp only [List.getCoeff]
    rw [show (buildCoeffsFromTermsNew p.terms
        ((p.terms[0]'(by omega)).monic.degree + 1)).toList[i]? =
        (buildCoeffsFromTermsNew p.terms
        ((p.terms[0]'(by omega)).monic.degree + 1))[i]? from by simp]
    simp only [buildCoeffsFromTermsNew]
    rw [← Array.foldl_toList]
    have hpw := pairwise_degree_ne_of_sorted_new p.terms p.sorted
    have hdegs := degrees_le_lead_new p.terms p.sorted h
    have hsize :
    ...
    · have hi_foldl : i < (p.terms.toList.foldl
      ...
      rw [foldl_set_getElem_new p.terms.toList hpw
    ...
    · have hge : (List.foldl (fun (acc : Array R) (m : MonomialNew 1 R ord) =>
