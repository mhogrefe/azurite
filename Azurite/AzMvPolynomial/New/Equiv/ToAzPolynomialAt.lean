...
import Azurite.AzMvPolynomial.New.Equiv.ToAzPolynomial
import Azurite.AzPolynomial.Equiv.Basic
import Mathlib.Algebra.MvPolynomial.Equiv

/-!
# Equivalence: `AzMvPolynomialNew.toAzPolynomialAt` ↔ Mathlib
...
open AzMvPolynomialNew MonomialOrder MvPolynomial Polynomial
...
private theorem mem_foldl_of_mem_init_new
    (l : List (MonomialNew n R ord)) (init : Finset (Fin n)) (w : Fin n) (hw : w ∈ init) :
...
omit [DecidableEq R] in
private theorem vars_sub_foldl_new
    (l : List (MonomialNew n R ord)) (init : Finset (Fin n)) (m : MonomialNew n R ord)
  ...
  | cons a tl ih =>
    ...
    · intro w hw
      exact mem_foldl_of_mem_init_new tl _ w (Finset.mem_union_right _ hw)
    · exact ih _ hm

omit [DecidableEq R] in
theorem MonomialNew.vars_sub_of_mem
    (p : AzMvPolynomialNew n R ord) (m : MonomialNew n R ord)
    (hm : m ∈ p.terms.toList) : m.vars ⊆ p.vars := by
  unfold AzMvPolynomialNew.vars; rw [← Array.foldl_toList]
  exact vars_sub_foldl_new _ _ _ hm

/-! ### Finsupp representation -/

/-- When `m.vars ⊆ {i}`, the finsupp is `single i (exponent i)`. -/
theorem MonicMonomialNew.toFinsupp_eq_single_exponent
    (m : MonicMonomialNew n ord) (i : Fin n) (hv : m.vars ⊆ {i}) :
  ...
  by_cases hw : w = i
  · subst hw; simp [MonicMonomialNew.exponent]
  · have hwv : w ∉ m.vars := fun hmem => hw (Finset.mem_singleton.mp (hv hmem))
    rw [MonicMonomialNew.vars_eq_toFinsupp_support] at hwv
    rw [Finsupp.notMem_support_iff] at hwv
    simp only [MonicMonomialNew.toFinsupp, Finsupp.onFinset_apply] at hwv
    rw [hwv, if_neg (Ne.symm hw)]

/-! ### eval₂ of a single monomial at a variable -/

omit [DecidableEq R] in
private theorem eval₂_monomial_at_var_new
    (m : MonicMonomialNew n ord) (c : R) (i : Fin n) (hv : m.vars ⊆ {i}) :
  ...
  rw [MonicMonomialNew.toFinsupp_eq_single_exponent m i hv, MvPolynomial.eval₂_monomial]
...
omit [DecidableEq R] in
private theorem eval₂_sum_map_toMvPoly_at_new
    (l : List (MonomialNew n R ord)) (i : Fin n)
    (hv : ∀ m ∈ l, (m : MonomialNew n R ord).monic.vars ⊆ {i}) :
    MvPolynomial.eval₂ Polynomial.C (fun _ => Polynomial.X)
  ...
  | cons m ms ih =>
    ...
    congr 1
    exact eval₂_monomial_at_var_new m.monic m.coeff.val i (hv m (List.mem_cons_self ..))

/-! ### Properties of the coefficient fold (generalized) -/

omit [DecidableEq R] in
private theorem foldl_set_size_list_at_new
    (l : List (MonomialNew n R ord)) (acc : Array R) (i : Fin n) :
...
omit [DecidableEq R] in
private theorem foldl_set_getElem_at_new (i : Fin n)
    ...
    result[k]'(by rw [foldl_set_size_list_at_new]; exact hk) =
...
omit [DecidableEq R] in
private theorem pairwise_exponent_ne_of_sorted_new (i : Fin n)
    (l : List (MonomialNew n R ord))
    (hsorted : l.Pairwise (fun a b => a.monic > b.monic))
    (hvars : ∀ m ∈ l, (m : MonomialNew n R ord).monic.vars ⊆ {i}) :
  ...
  | cons a as ih =>
    ...
    exact ne_of_gt (hsorted.1 b hb) (MonicMonomialNew.toFinsupp_injective
      (by rw [MonicMonomialNew.toFinsupp_eq_single_exponent _ i
                (hvars a (List.mem_cons_self ..)),
              MonicMonomialNew.toFinsupp_eq_single_exponent _ i
...
theorem MonicMonomialNew.eq_ofVarPow_of_vars_sub
    (m : MonicMonomialNew n ord) (i : Fin n) (hv : m.vars ⊆ {i}) :
    m = MonicMonomialNew.ofVarPow i (m.exponent i) := by
  ext1; ext j hj
  simp only [MonicMonomialNew.ofVarPow, Vector.getElem_ofFn]
  by_cases hij : i = ⟨j, hj⟩
  · simp [hij, MonicMonomialNew.exponent]
  · simp only [hij, ↓reduceIte]
    have hjne : (⟨j, hj⟩ : Fin n) ≠ i := fun h => hij h.symm
    have : (⟨j, hj⟩ : Fin n) ∉ m.vars :=
      fun hmem => hjne (Finset.mem_singleton.mp (hv hmem))
    rw [MonicMonomialNew.vars_eq_toFinsupp_support] at this
    rw [Finsupp.notMem_support_iff] at this
    simp [MonicMonomialNew.toFinsupp, Finsupp.onFinset_apply] at this
    exact this

/-! ### Exponent ordering from monomial ordering -/

omit [CommSemiring R] [DecidableEq R] in
private theorem exponent_le_of_monic_le_new (i : Fin n) (a b : MonicMonomialNew n ord)
    (hab : a ≤ b) (hav : a.vars ⊆ {i}) (hbv : b.vars ⊆ {i}) :
    a.exponent i ≤ b.exponent i := by
  rw [MonicMonomialNew.eq_ofVarPow_of_vars_sub a i hav,
      MonicMonomialNew.eq_ofVarPow_of_vars_sub b i hbv] at hab
  exact (MonicMonomialNew.ofVarPow_strictMono i).le_iff_le.mp hab

omit [DecidableEq R] in
private theorem exponents_le_lead_at_new (i : Fin n)
    (l : List (MonomialNew n R ord))
    (hsorted : l.Pairwise (fun a b => a.monic > b.monic))
    (hvars : ∀ m ∈ l, (m : MonomialNew n R ord).monic.vars ⊆ {i})
    (hne : l ≠ []) :
  ...
  · simp only [List.head_cons]
    ...
    · omega
    · exact Nat.lt_succ_of_le (exponent_le_of_monic_le_new i m.monic a.monic
        (le_of_lt ((List.pairwise_cons.mp hsorted).1 m hm))
        (hvars m (List.mem_cons_of_mem _ hm)) (hvars a (List.mem_cons_self ..)))

/-! ### Main equivalence theorem -/

/-- Converting an `AzMvPolynomialNew` to an `AzPolynomial` via `toAzPolynomialAt`
    and then to Mathlib's `Polynomial` agrees with applying `eval₂ C X`. -/
theorem toPoly_toAzPolynomialAt_new
    (p : AzMvPolynomialNew n R ord) {i : Fin n} (hv : p.vars ⊆ {i}) :
    AzPolynomial.toPoly (p.toAzPolynomialAt hv) =
    MvPolynomial.eval₂ Polynomial.C (fun _ => Polynomial.X) (toMvPoly p) := by
  have hvars : ∀ m ∈ p.terms.toList, (m : MonomialNew n R ord).monic.vars ⊆ {i} :=
    fun m hm => (MonomialNew.vars_sub_of_mem p m hm).trans hv
  rw [AzMvPolynomialNew.toMvPoly_eq_list_sum, eval₂_sum_map_toMvPoly_at_new _ i hvars]
  by_cases h : p.terms.size = 0
  · have hterms : p.terms = #[] := Array.eq_empty_of_size_eq_zero h
    simp [AzMvPolynomialNew.toAzPolynomialAt, AzPolynomial.toPoly, AzPolynomial.zero,
      List.toPoly, hterms]
  · unfold AzMvPolynomialNew.toAzPolynomialAt
    ...
    have hpw := pairwise_exponent_ne_of_sorted_new i p.terms.toList p.sorted hvars
    rw [sum_ite_eq_find_new p.terms.toList
      (fun m => m.monic.exponent i) (fun m => (m.coeff : R)) hpw k]
    simp only [List.getCoeff]
    rw [show (buildCoeffsFromTermsAtNew i p.terms
        (p.terms[0].monic.exponent i + 1)).toList[k]? =
        (buildCoeffsFromTermsAtNew i p.terms
        (p.terms[0].monic.exponent i + 1))[k]? from by simp]
    simp only [buildCoeffsFromTermsAtNew]
    rw [← Array.foldl_toList]
    have hne : p.terms.toList ≠ [] := by
      intro heq; exact h (by simp [Array.toList_eq_nil_iff.mp heq])
    have hdegs := exponents_le_lead_at_new i p.terms.toList p.sorted hvars hne
    ...
    have hfoldl_size : (p.terms.toList.foldl
        (fun (acc : Array R) (m : MonomialNew n R ord) =>
          let deg := m.monic.exponent i;
      ...
      rw [foldl_set_size_list_at_new]; simp
    ...
    · have hk_foldl : k < (p.terms.toList.foldl
      ...
      rw [foldl_set_getElem_at_new i p.terms.toList hpw
    ...
    · have hge : (List.foldl (fun (acc : Array R) (m : MonomialNew n R ord) =>
