/-
  Exponentiation for `MonicMonomialNew`, `MonomialNew`, `AzMvPolynomialNew`.
...
end Azurite.MonicMonomialNew

namespace Azurite.MonomialNew
...
def pow (m : MonomialNew n R ord) (k : ℕ) : MonomialNew n R ord :=
  ⟨⟨m.coeff.val ^ k, pow_ne_zero k m.coeff.property⟩, m.monic.pow k⟩

@[simp] theorem pow_coeff_val (m : MonomialNew n R ord) (k : ℕ) :
    (m.pow k).coeff.val = m.coeff.val ^ k := rfl

@[simp] theorem pow_monic (m : MonomialNew n R ord) (k : ℕ) :
    (m.pow k).monic = m.monic.pow k := rfl

end Azurite.MonomialNew

namespace Azurite.AzMvPolynomialNew
...
def pow (p : AzMvPolynomialNew n R ord) (k : ℕ) : AzMvPolynomialNew n R ord :=
  if h : p.terms.size = 1 then
    AzMvPolynomialNew.ofMonomial ((p.terms[0]'(by omega)).pow k)
  else
    Azurite.fastPow p k

end Azurite.AzMvPolynomialNew
