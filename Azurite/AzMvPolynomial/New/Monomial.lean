...
theorem eval_rename {n₂ : ℕ} [CommMonoidWithZero R]
    (m : MonomialNew n R ord) (f : Fin n → Fin n₂) (g : Fin n₂ → R) (ord₂ : MonomialOrder) :
    (m.rename f ord₂).eval g = m.eval (g ∘ f) := by
  simp only [eval, rename, MonicMonomialNew.eval_rename]
...
def mul [NoZeroDivisors R] (a b : MonomialNew n R ord) : MonomialNew n R ord :=
  ⟨⟨a.coeff.val * b.coeff.val, mul_ne_zero a.coeff.property b.coeff.property⟩,
   a.monic * b.monic⟩

instance [NoZeroDivisors R] : Mul (MonomialNew n R ord) := ⟨mul⟩

theorem eval_mul {R : Type _} [CommSemiring R] [NoZeroDivisors R]
    {n : ℕ} {ord : MonomialOrder}
    (a b : MonomialNew n R ord) (f : Fin n → R) :
  ...
  simp only [MonicMonomialNew.eval, MonicMonomialNew.mul_exponents,
             Fin.getElem_fin, Vector.getElem_ofFn, pow_add]
  rw [Finset.prod_mul_distrib]
  exact mul_mul_mul_comm _ _ _ _

instance [Nontrivial R] : One (MonomialNew n R ord) :=
  ⟨⟨⟨1, one_ne_zero⟩, MonicMonomialNew.one⟩⟩

@[ext] theorem ext' (a b : MonomialNew n R ord)
...
@[simp] theorem mul_coeff_val [NoZeroDivisors R] (a b : MonomialNew n R ord) :
    (a * b).coeff.val = a.coeff.val * b.coeff.val := rfl

set_option linter.unusedSectionVars false in
@[simp] theorem mul_monic [NoZeroDivisors R] (a b : MonomialNew n R ord) :
    (a * b).monic = a.monic * b.monic := rfl

set_option linter.unusedSectionVars false in
@[simp] theorem one_coeff_val [Nontrivial R] : (1 : MonomialNew n R ord).coeff.val = 1 := rfl

set_option linter.unusedSectionVars false in
@[simp] theorem one_monic [Nontrivial R] : (1 : MonomialNew n R ord).monic = 1 := rfl

variable {R : Type _} [CommSemiring R] [NoZeroDivisors R] [Nontrivial R]
    {n : ℕ} {ord : MonomialOrder}

omit [Nontrivial R] in
theorem mul_assoc (a b c : MonomialNew n R ord) : a * b * c = a * (b * c) := by
  apply ext' <;> simp [_root_.mul_assoc]

theorem one_mul (a : MonomialNew n R ord) : 1 * a = a := by
  apply ext' <;> simp

theorem mul_one (a : MonomialNew n R ord) : a * 1 = a := by
  apply ext' <;> simp

omit [Nontrivial R] in
theorem mul_comm (a b : MonomialNew n R ord) : a * b = b * a := by
  apply ext' <;> simp [_root_.mul_comm]

instance : CommMonoid (MonomialNew n R ord) where
  mul_assoc := mul_assoc
  one_mul := one_mul
  mul_one := mul_one
  mul_comm := mul_comm
...
def toCharsWith (m : MonomialNew n R ord) : List Char :=
  if m.monic = 1 then
...
/-- Parse a character list into a `MonomialNew` using the `F`-naming scheme. -/
def parseWith (cs : List Char) : Option (MonomialNew n R ord) :=
  match cs with
  | [] => none
  | c :: rest =>
    if decide (¬ isPolySyntaxChar c) then
      (MonicMonomialNew.parseWith (ord := ord) F cs).map
      ...
      | c' :: _ =>
        if decide (¬ isPolySyntaxChar c') then
          ...
          | some ⟨negOneVal, hne, _⟩ =>
            (MonicMonomialNew.parseWith (ord := ord) F rest).map
          ...
          | '*' :: monicPart =>
            (ParsableCoeff.parseChars coeffPart).bind (fun c =>
              if hc : c = 0 then none
              else (MonicMonomialNew.parseWith (ord := ord) F monicPart).map
                (fun m => ⟨⟨c, hc⟩, m⟩))
          | _ => none
      | [] =>
        let (coeffPart, rest') := cs.span (· != '*')
...
theorem toCharsWith_ne_nil (m : MonomialNew n R ord) : m.toCharsWith F ≠ [] := by
  unfold toCharsWith
  split_ifs with h1 h2
  · exact ParsableCoeff.toChars_nonempty _
  · exact MonicMonomialNew.toCharsWith_ne_nil F m.monic h1
  · cases hn : ParsableCoeff.negOne (R := R) with
    | none => intro h; simp at h
    | some c => dsimp only [hn]; split_ifs <;> (intro h; simp at h)

theorem plus_notin_toCharsWith (m : MonomialNew n R ord) : '+' ∉ m.toCharsWith F := by
  unfold toCharsWith
  split_ifs with h1 h2
  · intro h; exact ParsableCoeff.toChars_no_syntax _ _ h (Or.inl rfl)
  · exact MonicMonomialNew.plus_notin_toCharsWith F m.monic
  · cases hn : ParsableCoeff.negOne (R := R) with
    | none =>
      ...
      · simp at h
      · exact MonicMonomialNew.plus_notin_toCharsWith F m.monic h
    | some c =>
      dsimp only [hn]
      split_ifs with hc
      · intro h; simp at h; exact MonicMonomialNew.plus_notin_toCharsWith F m.monic h
      · intro h; rw [List.mem_append, List.mem_append] at h
        rcases h with (h | h) | h
        · exact ParsableCoeff.toChars_no_syntax _ _ h (Or.inl rfl)
        · simp at h
...
theorem minus_notin_tail_toCharsWith (m : MonomialNew n R ord) :
    '-' ∉ (m.toCharsWith F).tail := by
  unfold toCharsWith
  split_ifs with h1 h2
  · exact fun h => ParsableCoeff.toChars_no_minus_tail _ _ h rfl
  · exact fun h => MonicMonomialNew.minus_notin_toCharsWith F m.monic (List.mem_of_mem_tail h)
  · cases hn : ParsableCoeff.negOne (R := R) with
    | none =>
      ...
      · simp only [List.singleton_append, List.mem_cons] at h
        ...
        · exact MonicMonomialNew.minus_notin_toCharsWith F m.monic h
    | some c =>
      dsimp only [hn]
      split_ifs with hc
      · exact fun h => MonicMonomialNew.minus_notin_toCharsWith F m.monic h
      · have hne := ParsableCoeff.toChars_nonempty m.coeff.val
        rw [List.append_assoc, List.tail_append_of_ne_nil hne]
        intro h; rw [List.mem_append] at h
        rcases h with h | h
...
theorem MonomialNew.parseWith_toCharsWith {R : Type _} [Semiring R]
    [DecidableEq R] [ParsableCoeff R] {n : ℕ} {ord : MonomialOrder}
    (F : Type _) [LinearOrder F] [pv : ParsableVar F n]
    (m : MonomialNew n R ord) :
    MonomialNew.parseWith (n := n) (ord := ord) (R := R) F (m.toCharsWith F) = some m := by
  unfold MonomialNew.toCharsWith
  ...
  · -- Case 1: monic = 1, output is just the coefficient chars
    ...
    rw [hct]; unfold MonomialNew.parseWith
  ...
  · -- Case 3: coeff ≠ 1, monic ≠ 1
    suffices hCM : ∀ (hne : ParsableCoeff.toChars m.coeff.val ≠ []),
      ...
      | some ⟨cneg, hne_neg, hne_one⟩ =>
        ...
        · have hne_monic := MonicMonomialNew.toCharsWith_ne_nil F m.monic hmonic
          ...
          rw [hn]; simp only
          rw [← hct, MonicMonomialNew.parseWith_toCharsWith, Option.map_some]
          simp only [dite_true]
          cases m
          simp_all [MonomialNew.mk.injEq]
    ...
    · subst hcm; cases t with
      | nil =>
        ...
        simp_all [dif_neg m.coeff.property, MonicMonomialNew.parseWith_toCharsWith,
...
@[inline] def toChars [DecidableEq R] [ParsableCoeff R]
    (m : MonomialNew n R ord) : List Char :=
  m.toCharsWith (IndexedVar n)

/-- Default `parse`: accepts the `IndexedVar n` naming. -/
@[inline] def parse [DecidableEq R] [ParsableCoeff R]
    (cs : List Char) : Option (MonomialNew n R ord) :=
  parseWith (IndexedVar n) cs

theorem toChars_ne_nil [DecidableEq R] [ParsableCoeff R] (m : MonomialNew n R ord) :
    m.toChars ≠ [] :=
  toCharsWith_ne_nil (IndexedVar n) m

theorem plus_notin_toChars [DecidableEq R] [ParsableCoeff R] (m : MonomialNew n R ord) :
    '+' ∉ m.toChars :=
  plus_notin_toCharsWith (IndexedVar n) m

theorem minus_notin_tail_toChars [DecidableEq R] [ParsableCoeff R] (m : MonomialNew n R ord) :
    '-' ∉ m.toChars.tail :=
  minus_notin_tail_toCharsWith (IndexedVar n) m

theorem parse_toChars [DecidableEq R] [ParsableCoeff R] (m : MonomialNew n R ord) :
    parse (n := n) (ord := ord) (R := R) m.toChars = some m :=
  MonomialNew.parseWith_toCharsWith (IndexedVar n) m

end MonomialNew

section MonomialNewGuards

private def mkMonN (c : ℤ) (hc : c ≠ 0) (v : Vector ℕ 3) : MonomialNew 3 ℤ :=
  ⟨⟨c, hc⟩, ⟨v⟩⟩
...
#guard (MonomialNew.parse (n := 3) (R := ℤ) (ord := .Degrevlex) "5".toList).map
    MonomialNew.toChars == some "5".toList
#guard (MonomialNew.parse (n := 3) (R := ℤ) (ord := .Degrevlex) "x₀".toList).map
    MonomialNew.toChars == some "x₀".toList
#guard (MonomialNew.parse (n := 3) (R := ℤ) (ord := .Degrevlex) "3*x₀".toList).map
    MonomialNew.toChars == some "3*x₀".toList
#guard (MonomialNew.parse (n := 3) (R := ℤ) (ord := .Degrevlex) "-2*x₀*x₁".toList).map
    MonomialNew.toChars == some "-2*x₀*x₁".toList

-- parse round-trip via `AbcVar 3`
#guard (MonomialNew.parseWith (n := 3) (R := ℤ) (ord := .Degrevlex) (AbcVar 3) "3*a".toList).map
    (fun m => m.toCharsWith (AbcVar 3)) == some "3*a".toList

end MonomialNewGuards
