...
instance {R : Type _} [Semiring R] [DecidableEq R] {n : ℕ} {ord : MonomialOrder} :
    DecidableEq (AzMvPolynomialNew n R ord) :=
  fun a b => by
    have : DecidableEq (Array (MonomialNew n R ord)) := inferInstance
    cases a; cases b
    simp only [AzMvPolynomialNew.mk.injEq]
    exact this _ _

namespace AzMvPolynomialNew

variable {R : Type _} [Semiring R] {n : ℕ} {ord : MonomialOrder}

/-- The zero polynomial (empty term list). -/
def zero : AzMvPolynomialNew n R ord := ⟨#[], List.Pairwise.nil⟩

instance : Zero (AzMvPolynomialNew n R ord) := ⟨zero⟩

/-- A polynomial is zero iff its term array is empty. -/
@[simp] theorem zero_terms : (0 : AzMvPolynomialNew n R ord).terms = #[] := rfl

/-- Number of terms in the polynomial. -/
def numTerms (p : AzMvPolynomialNew n R ord) : ℕ := p.terms.size

/-- The leading monomial (largest monic part), if the polynomial is nonzero. -/
def leadTerm (p : AzMvPolynomialNew n R ord) : Option (MonomialNew n R ord) :=
  p.terms[0]?

/-- The leading monic monomial, if the polynomial is nonzero. -/
def leadMonic (p : AzMvPolynomialNew n R ord) : Option (MonicMonomialNew n ord) :=
  p.leadTerm.map MonomialNew.monic

/-- The leading coefficient, if the polynomial is nonzero. -/
def leadCoeff (p : AzMvPolynomialNew n R ord) : Option {c : R // c ≠ 0} :=
  p.leadTerm.map MonomialNew.coeff

/-- Construct a polynomial from a single monomial. -/
def ofMonomial (m : MonomialNew n R ord) : AzMvPolynomialNew n R ord :=
  ⟨#[m], List.pairwise_singleton _ _⟩

/-- The constant polynomial `1`.  Returns `0` when `1 = 0` in the
    coefficient ring (trivial ring). -/
def one [DecidableEq R] : AzMvPolynomialNew n R ord :=
  if h : (1 : R) = 0 then 0
  else ofMonomial (MonomialNew.one h)

instance [DecidableEq R] : One (AzMvPolynomialNew n R ord) := ⟨one⟩

/-- Constructs a constant polynomial with value `c`.
    Returns `0` when `c = 0`. -/
def C [DecidableEq R] (c : R) : AzMvPolynomialNew n R ord :=
  if h : c = 0 then 0
  else ofMonomial ⟨⟨c, h⟩, MonicMonomialNew.one⟩

/-- The polynomial consisting of a single variable `i : Fin n`.
    Returns `0` when `1 = 0` in the trivial ring. -/
def X [DecidableEq R] (i : Fin n) : AzMvPolynomialNew n R ord :=
  if h : (1 : R) = 0 then 0
  else ofMonomial ⟨⟨1, h⟩, MonicMonomialNew.ofVar i⟩

/-- The total degree of the polynomial (maximum total degree among its terms),
    or 0 for the zero polynomial. -/
def totalDegree (p : AzMvPolynomialNew n R ord) : ℕ :=
...
private theorem withOrder_monic_ne_of_perm
  ...
  exact p.sorted.imp (fun {a b} hab heq => by
    apply ne_of_gt hab
    simp only [MonomialNew.withOrder, MonicMonomialNew.withOrder] at heq
    exact MonicMonomialNew.ext (MonicMonomialNew.mk.inj heq))
...
def withOrder (p : AzMvPolynomialNew n R ord) (ord' : MonomialOrder) :
...
def ofMonomials (ms : Array (MonomialNew n R ord))
...
def adjacentDistinct : List (MonomialNew n R ord) → Bool
  | [] => true
  | [_] => true
...
def ofMonomials? (ms : Array (MonomialNew n R ord)) :
    Option (AzMvPolynomialNew n R ord) :=
  let sorted := ms.toList.mergeSort monicGeq
