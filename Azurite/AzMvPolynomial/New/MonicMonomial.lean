/-
  ...
  `MonicMonomialNew n ord` is the Fin-indexed replacement for the
...
@[ext]
structure MonicMonomialNew (n : ℕ) (ord : MonomialOrder := .Degrevlex) where
...
def eval {R : Type _} [CommMonoid R] (m : MonicMonomialNew n ord) (f : Fin n → R) : R :=
...
def rename {n₂ : ℕ} (m : MonicMonomialNew n ord) (f : Fin n → Fin n₂)
    (ord₂ : MonomialOrder := ord) : MonicMonomialNew n₂ ord₂ :=
  ⟨Vector.ofFn (fun j : Fin n₂ =>
    Finset.univ.sum (fun i : Fin n =>
      if f i = j then m.exponents[i] else 0))⟩

/-- Multiply two monic monomials (add exponents pointwise). -/
def mul (a b : MonicMonomialNew n ord) : MonicMonomialNew n ord :=
  ⟨Vector.ofFn (fun i => a.exponents[i] + b.exponents[i])⟩

instance : One (MonicMonomialNew n ord) := ⟨one⟩
instance : Mul (MonicMonomialNew n ord) := ⟨mul⟩

instance : Ord (MonicMonomialNew n ord) where
  compare a b := ord.compareExponents a.exponents b.exponents

@[simp] theorem one_exponents :
    (1 : MonicMonomialNew n ord).exponents = Vector.replicate n 0 := rfl
@[simp] theorem mul_exponents (a b : MonicMonomialNew n ord) :
    (a * b).exponents = Vector.ofFn (fun i => a.exponents[i] + b.exponents[i]) := rfl

theorem mul_assoc (a b c : MonicMonomialNew n ord) : a * b * c = a * (b * c) := by
  ext1; simp only [mul_exponents]; ext i hi; simp; omega

theorem one_mul (a : MonicMonomialNew n ord) : 1 * a = a := by
  ext1; simp only [mul_exponents, one_exponents]; ext i hi; simp

theorem mul_one (a : MonicMonomialNew n ord) : a * 1 = a := by
  ext1; simp only [mul_exponents, one_exponents]; ext i hi; simp

theorem mul_comm (a b : MonicMonomialNew n ord) : a * b = b * a := by
  ext1; simp only [mul_exponents]; ext i hi; simp [Nat.add_comm]

instance : CommMonoid (MonicMonomialNew n ord) where
  mul_assoc := mul_assoc
  one_mul := one_mul
  mul_one := mul_one
  mul_comm := mul_comm
...
def withOrder (m : MonicMonomialNew n ord) (ord' : MonomialOrder) :
    MonicMonomialNew n ord' :=
  ⟨m.exponents⟩

@[simp] theorem withOrder_exponents (m : MonicMonomialNew n ord) (ord' : MonomialOrder) :
...
def toCharsWith (m : MonicMonomialNew n ord) : List Char :=
...
def parseWith (cs : List Char) : Option (MonicMonomialNew n ord) :=
...
@[inline] def toChars (m : MonicMonomialNew n ord) : List Char :=
  m.toCharsWith (IndexedVar n)

/-- Default `parse`: accepts the `IndexedVar n` naming (`x₀, x₁, …`). -/
@[inline] def parse (cs : List Char) : Option (MonicMonomialNew n ord) :=
  parseWith (IndexedVar n) cs

instance : ToString (MonicMonomialNew n ord) where
  toString m := String.ofList m.toChars

instance : Repr (MonicMonomialNew n ord) where
...
section MonicMonomialNewGuards

private def mmN (v : Vector ℕ 3) : MonicMonomialNew 3 := ⟨v⟩
...
#guard MonicMonomialNew.parseWith (n := 3) (ord := .Degrevlex) (AbcVar 3) "a".toList
    == some (mmN (Vector.mk #[1, 0, 0] rfl))
#guard MonicMonomialNew.parseWith (n := 3) (ord := .Degrevlex) (AbcVar 3) "c*a".toList
    == some (mmN (Vector.mk #[1, 0, 1] rfl))

-- default `parse` uses `IndexedVar`
#guard MonicMonomialNew.parse (n := 3) (ord := .Degrevlex) "x₀*x₂^3".toList
    == some (mmN (Vector.mk #[1, 0, 3] rfl))

end MonicMonomialNewGuards
