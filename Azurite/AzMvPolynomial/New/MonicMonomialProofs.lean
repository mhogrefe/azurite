...
private def toCharsAuxWith (m : MonicMonomialNew n ord) (k : ℕ) : List (List Char) :=
  if h : k < n then
...
private def mkFactorWith (m : MonicMonomialNew n ord) (i : Fin n) : Option (List Char) :=
...
private theorem parseFactorListWith_toCharsAuxWith
    (m : MonicMonomialNew n ord) (k : ℕ) (hk : k ≤ n) (exps : Vector ℕ n)
...
private theorem toCharsAuxWith_eq (m : MonicMonomialNew n ord) (k : ℕ) (hk : k ≤ n) :
...
private theorem toCharsWith_eq_intercalate (m : MonicMonomialNew n ord) :
...
private theorem star_notin_toCharsAuxWith (m : MonicMonomialNew n ord) (k : ℕ) (_hk : k ≤ n) :
...
private theorem toCharsAuxWith_nil_of_zero (m : MonicMonomialNew n ord) (k : ℕ) (_hk : k ≤ n)
...
private theorem toCharsAuxWith_nil_imp (m : MonicMonomialNew n ord) (k : ℕ) (hk : k ≤ n) :
...
private theorem toCharsAuxWith_factors_nonempty (m : MonicMonomialNew n ord) (k : ℕ) (_hk : k ≤ n) :
...
theorem toCharsWith_ne_nil (m : MonicMonomialNew n ord) (hm : m ≠ 1) :
    m.toCharsWith F ≠ [] := by
  have hem : m.exponents ≠ Vector.replicate n 0 := by
    intro he; exact hm (MonicMonomialNew.ext he)
...
private theorem toCharsAuxWith_factors_head_not_syntax (m : MonicMonomialNew n ord) (k : ℕ) :
...
private theorem plus_minus_notin_toCharsAuxWith (m : MonicMonomialNew n ord) (k : ℕ) (_hk : k ≤ n) :
...
theorem plus_notin_toCharsWith (m : MonicMonomialNew n ord) : '+' ∉ m.toCharsWith F := by
...
theorem minus_notin_toCharsWith (m : MonicMonomialNew n ord) : '-' ∉ m.toCharsWith F := by
  rw [toCharsWith_eq_intercalate]
  exact not_mem_intercalate (by simp)
...
theorem parseWith_toCharsWith (m : MonicMonomialNew n ord) :
...
theorem toChars_ne_nil (m : MonicMonomialNew n ord) (hm : m ≠ 1) : m.toChars ≠ [] :=
  toCharsWith_ne_nil (IndexedVar n) m hm

theorem toChars_head_not_syntax (m : MonicMonomialNew n ord) (hm : m ≠ 1) :
    ¬ Azurite.isPolySyntaxChar (m.toChars.head (toChars_ne_nil m hm)) :=
  toCharsWith_head_not_syntax (IndexedVar n) m hm

theorem plus_notin_toChars (m : MonicMonomialNew n ord) : '+' ∉ m.toChars :=
  plus_notin_toCharsWith (IndexedVar n) m

theorem minus_notin_toChars (m : MonicMonomialNew n ord) : '-' ∉ m.toChars :=
  minus_notin_toCharsWith (IndexedVar n) m

theorem parse_toChars (m : MonicMonomialNew n ord) :
...
theorem eval_rename {n₂ : ℕ} {R : Type _} [CommMonoid R]
    (m : MonicMonomialNew n ord) (f : Fin n → Fin n₂) (g : Fin n₂ → R)
    (ord₂ : MonomialOrder) :
...
theorem eval_mul {R : Type _} [CommMonoid R]
    (a b : MonicMonomialNew n ord) (f : Fin n → R) :
