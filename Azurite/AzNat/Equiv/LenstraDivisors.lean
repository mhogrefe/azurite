...
private theorem stateBridge_lstep (σ : LenstraState) :
  ...
  rw [lstep, CP.lstep]
  by_cases h0 : σ.a0 = 0
  · rw [ite_eq_left h0, ite_eq_left (show (stateBridge σ).a0 = 0 from
      (eq_zero_iff _).mp h0)]
  · rw [ite_eq_right h0, ite_eq_right (show ¬(stateBridge σ).a0 = 0 from
    ...
    · rw [ite_eq_left hγ, ite_eq_left (show (stateBridge σ).a0 % (stateBridge σ).a1
    ...
    · rw [ite_eq_right hγ, ite_eq_right (show ¬(stateBridge σ).a0 % (stateBridge σ).a1
...
private theorem toInt_lA (s a₁ : AzNat) (c₁ : AzInt) (i : ℕ) :
  ...
  rw [lA, CP.lA]
  by_cases h : i % 2 = 0
  · rw [ite_eq_left h, ite_eq_left h, AzNat.toInt_toAzInt, lchain_a0_bridge]
  · rw [ite_eq_right h, ite_eq_right h, AzNat.toInt_toAzInt,
...
private theorem toInt_lB (s a₁ : AzNat) (c₁ : AzInt) (i : ℕ) :
  ...
  rw [lB, CP.lB]
  by_cases h : i % 2 = 0
  · rw [ite_eq_left h, ite_eq_left h, show (lchain s a₁ c₁ (i / 2)).b0.toInt
      = (CP.lchain s.toNat a₁.toNat c₁.toInt (i / 2)).b0 from by
      rw [← stateBridge_lchain]; rfl]
  · rw [ite_eq_right h, ite_eq_right h, show (lchain s a₁ c₁ (i / 2)).b1.toInt
...
private theorem toInt_lC (s a₁ : AzNat) (c₁ : AzInt) (i : ℕ) :
  ...
  rw [lC, CP.lC]
  by_cases h : i % 2 = 0
  · rw [ite_eq_left h, ite_eq_left h, show (lchain s a₁ c₁ (i / 2)).c0.toInt
      = (CP.lchain s.toNat a₁.toNat c₁.toInt (i / 2)).c0 from by
      rw [← stateBridge_lchain]; rfl]
  · rw [ite_eq_right h, ite_eq_right h, show (lchain s a₁ c₁ (i / 2)).c1.toInt
...
private theorem intSqrt?_bridge (D : AzInt) :
  ...
  by_cases hneg : D < 0
  · rw [ite_eq_left hneg, ite_eq_left ((azint_lt_zero_iff D).mp hneg)]
    rfl
  · rw [ite_eq_right hneg, ite_eq_right (fun h => hneg ((azint_lt_zero_iff D).mpr h))]
    ...
    · have hrem' : D.natAbs.toNat
      ...
      rw [ite_eq_left hrem, ite_eq_left (by rw [← hDt]; omega)]
      rw [Option.map_some]
      congr 1
    ...
    · have hrem' : ¬(D.natAbs.toNat
      ...
      rw [ite_eq_right hrem, ite_eq_right (by rw [← hDt]; omega)]
      rfl
...
private theorem map_toNat_reportIfDivisor (n r s : AzNat) (u : AzInt) :
  ...
  · have hpos' : 0 < u.toInt := (azint_pos_iff u).mp hpos
    ...
    · rw [ite_eq_left ⟨hpos, hc.1, hc.2⟩,
        ite_eq_left ⟨hpos', (hciff.mp hc).1, (hciff.mp hc).2⟩,
        List.map_cons, List.map_nil, hna]
    · rw [ite_eq_right (fun h => hc ⟨h.2.1, h.2.2⟩),
        ite_eq_right (fun h => hc (hciff.mpr ⟨h.2.1, h.2.2⟩))]
      rfl
  · rw [ite_eq_right (fun h => hpos h.1),
      ite_eq_right (fun h => hpos ((azint_pos_iff u).mpr h.1))]
    rfl
...
private theorem map_toNat_solveSystem (n r r' s : AzNat) (a b c : AzInt) :
  ...
  by_cases ha : a = 0
  · rw [ite_eq_left ha, ite_eq_left ((azint_eq_zero_iff a).mp ha)]
    by_cases hb : b = 0
    · rw [ite_eq_left hb, ite_eq_left ((azint_eq_zero_iff b).mp hb)]
      rfl
    · rw [ite_eq_right hb, ite_eq_right (fun h => hb ((azint_eq_zero_iff b).mpr h))]
      ...
      · rw [ite_eq_left hc1, ite_eq_left (hcond1.mp hc1)]
        ...
        · have hv' := hposiff.mp hv
          ...
          · rw [ite_eq_left ⟨hv, hd⟩, ite_eq_left ⟨hv', hdvdiff.mp hd⟩,
              map_toNat_reportIfDivisor]
            congr 1
            rw [AzNat.toInt_toAzInt, toNat_div, hna]
          · rw [ite_eq_right (fun h => hd h.2),
              ite_eq_right (fun h => hd (hdvdiff.mpr h.2))]
            rfl
        · rw [ite_eq_right (fun h => hv h.1),
            ite_eq_right (fun h => hv (hposiff.mpr h.1))]
          rfl
      · rw [ite_eq_right hc1, ite_eq_right (fun h => hc1 (hcond1.mpr h))]
        rfl
  · rw [ite_eq_right ha, ite_eq_right (fun h => ha ((azint_eq_zero_iff a).mpr h))]
    ...
    · have hcp : CP.intSqrt? ((c.toInt * (s.toNat : ℤ)
      ...
      · by_cases h1 : (c * s.toAzInt + a * r.toAzInt + b * r'.toAzInt
            + w) % (2 * a) = 0
        · rw [ite_eq_left h1, ite_eq_left (hbr1.mp h1), map_toNat_reportIfDivisor]
          congr 1
          rw [AzInt.toInt_hDiv, toInt_add, hR, h2a]
        · rw [ite_eq_right h1, ite_eq_right (fun h => h1 (hbr1.mpr h))]
          rfl
      · by_cases h2 : (c * s.toAzInt + a * r.toAzInt + b * r'.toAzInt
            - w) % (2 * a) = 0
        · rw [ite_eq_left h2, ite_eq_left (hbr2.mp h2), map_toNat_reportIfDivisor]
          congr 1
          rw [AzInt.toInt_hDiv, toInt_sub, hR, h2a]
        · rw [ite_eq_right h2, ite_eq_right (fun h => h2 (hbr2.mpr h))]
          rfl
...
private theorem map_toInt_evenWindow (s : AzNat) (cᵢ : AzInt) :
  ...
  · rw [ite_eq_left h, ite_eq_left (by rw [← hm]; exact (azint_eq_zero_iff _).mp h)]
    rfl
  · rw [ite_eq_right h, ite_eq_right (by
      rw [← hm]
...
private theorem map_toInt_oddWindow (n s : AzNat) (a b cᵢ : AzInt) :
  ...
  by_cases hcond : s.toAzInt ^ 2 * ((2 * (a * b)
      + (cᵢ - 2 * (a * b)) % s.toAzInt + (ofNat j).toAzInt * s.toAzInt)
        - a * b) < n.toAzInt
  · rw [ite_eq_left hcond, ite_eq_left (hciff.mp hcond), Option.map_some, hcexp]
  · rw [ite_eq_right hcond, ite_eq_right (fun h => hcond (hciff.mpr h))]
    rfl
