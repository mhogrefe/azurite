...
lemma testBit_toNatLimbsList_aux (q r : Nat) (hr : r < 64) :
  ...
  | zero =>
    ...
    | cons x xs =>
      ...
      simp only [Nat.mul_zero, Nat.add_zero, hr, if_true,
        List.length_cons, Nat.zero_lt_succ, dif_pos, List.getElem_cons_zero]
  | succ k ih =>
    intro l
    cases l with
    | nil => simp [toNatLimbsList]
    | cons x xs =>
      ...
      · have h_cons_lt : k + 1 < (x :: xs).length := by simp; omega
        simp only [hk, dif_pos, h_cons_lt, dif_pos, List.getElem_cons_succ]
      · have h_cons_not_lt : ¬ k + 1 < (x :: xs).length := by
          simp only [List.length_cons, not_lt]; omega
        simp only [hk, dif_neg, not_false_eq_true, h_cons_not_lt]
...
theorem testBit_eq_toNat_testBit (n : AzNat) (i : Nat) :
  ...
  · have h' : ¬ i / 64 < n.limbs.toList.length := fun hc => h (h_size_eq ▸ hc)
    simp only [h, dif_neg, not_false_eq_true, h', dif_neg]
