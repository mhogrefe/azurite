...
private def toNatLimbsList (l : List UInt64) : Nat :=
  l.foldr (fun limb acc => acc * (2 ^ 64) + limb.toNat) 0

private lemma toNatLimbsList_cons (x : UInt64) (xs : List UInt64) :
  toNatLimbsList (x :: xs) = toNatLimbsList xs * 2^64 + x.toNat := rfl

private lemma toNatLimbsList_append (l1 l2 : List UInt64) :
