...
/-!
## Correctness of `AzNat.squareToomCook3ModPow2`

`(squareToomCook3ModPow2 t a k).toNat = a.toNat ^ 2 % 2 ^ k`.
...
/-- **Correctness of `squareToomCook3ModPow2`.** -/
theorem toNat_squareToomCook3ModPow2 (toomThreshold karaThreshold : Nat) (a : AzNat) (k : Nat) :
    (squareToomCook3ModPow2 toomThreshold karaThreshold a k).toNat = a.toNat ^ 2 % 2 ^ k := by
  simp only [squareToomCook3ModPow2]
  ...
  · -- Recursive: full-square corner + doubled low cross.
    rename_i h
    set s := min ((4 * L + 4) / 5) (L - 1) with hsdef
    ...
    set cornerFull := ofLimbs (toomCook3SquareLimbs toomThreshold karaThreshold aPad 0 s hbA) with hcdef
    set cross := toomCook3MulLowLimbs toomThreshold karaThreshold aPad aPad 0 s m hbC1 hbC2 with hcrdef
    ...
    have hcorner : cornerFull.toNat = A0 ^ 2 := by
      rw [hcdef, toNat_ofLimbs, toomCook3SquareLimbs_toNat]
    have hcross : cross.toNat ≡ A0lo * A1 [MOD 2 ^ (64 * m)] := by
      rw [hcrdef]; exact toomCook3MulLowLimbs_modEq toomThreshold karaThreshold m aPad aPad 0 s hbC1 hbC2
