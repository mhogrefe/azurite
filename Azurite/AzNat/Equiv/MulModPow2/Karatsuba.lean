...
/-!
## Correctness of `AzNat.mulKaratsubaModPow2`

`(mulKaratsubaModPow2 threshold a b k).toNat = (a.toNat * b.toNat) % 2 ^ k`.

The recursion correctness is a short-product invariant modulo `2 ^ (64·len)`: the
dropped `A₁·B₁·β^(2k)` term is `≡ 0` (`2k ≥ len`), the recursive cross products
agree modulo `2 ^ (64·m)`, and the assembly is one full multiply plus a shifted
add — all via the already-proven `addModPow2`, full Karatsuba, and schoolbook-low.
-/
...
/-- **Short-product recursion correctness.** `karatsubaMulLowLimbs` computes the
    two slices' product truncated to `2 ^ (64·len)`. -/
theorem karatsubaMulLowLimbs_modEq (threshold : Nat) :
    ∀ (len : Nat) (a b : Array UInt64) (loA loB : Nat)
      (hA : loA + len ≤ a.size) (hB : loB + len ≤ b.size),
      (karatsubaMulLowLimbs threshold a b loA loB len hA hB).toNat
  ...
  · have hlen : 2 ≤ len := by omega
    ...
    set C0 := ofLimbs (karatsubaMulLimbs threshold a b loA loB k hA0 hB0) with hC0_def
    set midA := karatsubaMulLowLimbs threshold a b loA (loB + k) m hMidA_a hMidA_b with hmidA_def
    set midB := karatsubaMulLowLimbs threshold a b (loA + k) loB m hMidB_a hMidB_b with hmidB_def
    ...
    have heq : karatsubaMulLowLimbs threshold a b loA loB len hA hB
        = addModPow2 C0 shifted (64 * len) := by
      rw [karatsubaMulLowLimbs]
      split
    ...
    have hC0 : C0.toNat = toNatLimbsList ((a.toList.drop loA).take k)
      ...
      rw [hC0_def, toNat_ofLimbs, karatsubaMulLimbs_toNat]
...
/-- **Correctness of `mulKaratsubaModPow2`.** -/
theorem toNat_mulKaratsubaModPow2 (threshold : Nat) (a b : AzNat) (k : Nat) :
    (mulKaratsubaModPow2 threshold a b k).toNat = (a.toNat * b.toNat) % 2 ^ k := by
  unfold mulKaratsubaModPow2
  rw [toNat_modPow2]
  ...
  have hmod := karatsubaMulLowLimbs_modEq threshold L aPad bPad 0 0 hbndA hbndB
