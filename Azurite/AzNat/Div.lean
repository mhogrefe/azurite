...
/-- Size preservation of `schoolbookDivModLimbs`. -/
theorem schoolbookDivModLimbs_size (a b : Array UInt64) (loA loB n m : Nat)
    (h_n_pos : 0 < n) (hA : loA + n + m ≤ a.size) (hB : loB + n ≤ b.size)
    (hbn1 : 2 ^ 63 ≤ (b[loB + n - 1]'(by omega)).toNat) :
    (schoolbookDivModLimbs a b loA loB n m h_n_pos hA hB hbn1).1.size = a.size := by
