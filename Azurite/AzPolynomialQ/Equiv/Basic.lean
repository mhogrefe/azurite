...
This module defines the conversions between `AzPolynomialQ` (integer numerators with
a shared positive denominator, in canonical form) and `AzPolynomial ℚ` (array of rationals).

## Main definitions

- `AzPolynomialQ.toIntPoly` — the underlying `AzPolynomial ℤ` of numerators
- `AzPolynomialQ.toAzPolynomial` — forward conversion: `n_i ↦ n_i / denom`
- `AzPolynomialQ.ofAzPolynomial` — inverse: scale coefficients by LCM of denominators, then normalize

## Main results

- `coeff_toAzPolynomial` — `toAzPolynomial p` has the same rational coefficients as `p`
- `toAzPolynomial_zero` — `toAzPolynomial 0 = 0`
- `coeff_ofAzPolynomial` — `ofAzPolynomial p` has the same rational coefficients as `p`
- `toAzPolynomial_ofAzPolynomial` — roundtrip `AzPolynomial ℚ → AzPolynomialQ → AzPolynomial ℚ = id`
-/
...
/-- The `AzPolynomial ℤ` of numerators underlying a `AzPolynomialQ`. -/
def AzPolynomialQ.toIntPoly (p : AzPolynomialQ) : AzPolynomial ℤ :=
  ⟨p.numerators, p.last_ne_zero⟩

/-- Convert a `AzPolynomialQ` to a `AzPolynomial ℚ`: map each numerator `n_i ↦ n_i / denom`.
    Uses `mapZeroInjective` so the result is automatically normalized. -/
def AzPolynomialQ.toAzPolynomial (p : AzPolynomialQ) : AzPolynomial ℚ :=
  mapZeroInjective
    (fun n : ℤ => (n : ℚ) / p.denom)
    (fun n => ⟨fun h => by
...
@[simp]
lemma AzPolynomialQ.coeff_toAzPolynomial (p : AzPolynomialQ) (i : ℕ) :
    p.toAzPolynomial.coeff i = p.coeff i := by
  simp only [toAzPolynomial, toIntPoly, mapZeroInjective, AzPolynomial.coeff, coeff, Array.getElem?_map]
  cases p.numerators[i]? with
  | none => simp
  | some n => simp

@[simp]
lemma AzPolynomialQ.toAzPolynomial_zero : (0 : AzPolynomialQ).toAzPolynomial = 0 := by
  show mapZeroInjective (fun n : ℤ => (n : ℚ) / (1 : ℕ)) _
    (⟨#[], by simp⟩ : AzPolynomial ℤ) = (⟨#[], by simp⟩ : AzPolynomial ℚ)
  simp [mapZeroInjective]

/-! ## ofAzPolynomial -/

/-- Convert a `AzPolynomial ℚ` to a canonical `AzPolynomialQ`:
...
def AzPolynomialQ.ofAzPolynomial (p : AzPolynomial ℚ) : AzPolynomialQ :=
  let l := p.coeffs.toList
  let d := listLcm' (l.map (·.den))
  AzPolynomialQ.normalize (l.map (fun q => scaleRat q d)).toArray d (listLcm'_denList_pos l)

/-! ## Roundtrip: toAzPolynomial ∘ ofAzPolynomial = id -/

/-- The rational coefficients of `ofAzPolynomial p` agree with those of `p`. -/
@[simp]
lemma AzPolynomialQ.coeff_ofAzPolynomial (p : AzPolynomial ℚ) (i : ℕ) :
    (ofAzPolynomial p).coeff i = p.coeff i := by
  simp only [ofAzPolynomial]
  rw [AzPolynomialQ.normalize_coeff]
  ...
  simp only [scaleNums, AzPolynomial.coeff, ← Array.getElem?_toList, List.getElem?_map]
...
/-- The roundtrip `AzPolynomial ℚ → AzPolynomialQ → AzPolynomial ℚ` is the identity. -/
lemma AzPolynomialQ.toAzPolynomial_ofAzPolynomial (p : AzPolynomial ℚ) :
    (ofAzPolynomial p).toAzPolynomial = p := by
  apply_fun AzPolynomial.toPoly using fun p q h => toPoly_inj.mp h
  ext i
  simp [coeff_toPoly_eq, coeff_toAzPolynomial, coeff_ofAzPolynomial]
...
private lemma denom_dvd_of_cross (p q : AzPolynomialQ)
...
/-- Two canonical `AzPolynomialQ` with the same rational coefficient at every position are equal.
    Uniqueness of reduced-fraction representation with a shared denominator. -/
lemma AzPolynomialQ.coeff_ext (p q : AzPolynomialQ)
    (h : ∀ i, p.coeff i = q.coeff i) : p = q := by
  have cross : ∀ (i : ℕ), (p.numerators[i]?.getD 0 : ℤ) * q.denom =
      (q.numerators[i]?.getD 0 : ℤ) * p.denom := by
    intro i; have hi := h i; simp only [AzPolynomialQ.coeff] at hi
  ...
  exact AzPolynomialQ.ext hnums hdenom

/-- The roundtrip `AzPolynomialQ → AzPolynomial ℚ → AzPolynomialQ` is the identity. -/
lemma AzPolynomialQ.ofAzPolynomial_toAzPolynomial (p : AzPolynomialQ) :
    ofAzPolynomial p.toAzPolynomial = p := by
...
/-- Convert a `AzPolynomialQ` to a `Polynomial ℚ` by composing through `AzPolynomial ℚ`. -/
noncomputable def AzPolynomialQ.toPoly (p : AzPolynomialQ) : Polynomial ℚ :=
  AzPolynomial.toPoly p.toAzPolynomial

@[simp] lemma AzPolynomialQ.coeff_toPoly_eq (p : AzPolynomialQ) (i : ℕ) :
    p.toPoly.coeff i = p.coeff i := by
  simp only [AzPolynomialQ.toPoly, _root_.coeff_toPoly_eq, coeff_toAzPolynomial]

@[simp] lemma AzPolynomialQ.natDegree_toPoly (p : AzPolynomialQ) :
    p.toPoly.natDegree = p.natDegree := by
  simp [toPoly, AzPolynomial.natDegree_toPoly, toAzPolynomial, toIntPoly,
    mapZeroInjective, AzPolynomialQ.natDegree, AzPolynomial.natDegree]

@[simp] lemma AzPolynomialQ.degree_toPoly (p : AzPolynomialQ) :
    p.toPoly.degree = p.degree := by
  simp only [AzPolynomialQ.degree]
  split
  · case isTrue h =>
    have : p.toPoly = 0 := by
      ext i; simp [coeff_toPoly_eq, AzPolynomialQ.coeff, h]
    simp [this]
  · case isFalse h =>
    have hne : p.toPoly ≠ 0 := by
      ...
      simp [coeff_toPoly_eq, AzPolynomialQ.coeff,
...
@[simp] lemma AzPolynomialQ.leadingCoeff_toPoly (p : AzPolynomialQ) :
    p.toPoly.leadingCoeff = p.leadingCoeff := by
  simp [Polynomial.leadingCoeff, AzPolynomialQ.leadingCoeff,
    coeff_toPoly_eq, natDegree_toPoly]

@[simp] lemma AzPolynomialQ.nextCoeff_toPoly (p : AzPolynomialQ) :
    p.toPoly.nextCoeff = p.nextCoeff := by
  simp only [Polynomial.nextCoeff, AzPolynomialQ.nextCoeff, natDegree_toPoly]
  split
  · rfl
  · exact coeff_toPoly_eq p _

@[simp] lemma AzPolynomialQ.Monic_toPoly (p : AzPolynomialQ) :
    p.toPoly.Monic ↔ p.Monic := by
  simp [Polynomial.Monic, AzPolynomialQ.Monic, leadingCoeff_toPoly]

@[simp] lemma AzPolynomialQ.toPoly_zero :
    (0 : AzPolynomialQ).toPoly = 0 := by
  ext i; simp [coeff_toPoly_eq, AzPolynomialQ.coeff]

@[simp] lemma AzPolynomialQ.toPoly_one :
    (1 : AzPolynomialQ).toPoly = 1 := by
  ext i; simp [coeff_toPoly_eq, AzPolynomialQ.coeff]
  cases i with
  | zero => simp
