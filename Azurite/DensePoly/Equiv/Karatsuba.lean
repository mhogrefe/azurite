...
import Azurite.DensePoly.Equiv.Sub
import Azurite.DensePoly.Equiv.Mul

/-!
# Correctness Proof for Karatsuba Multiplication

Establishes `toPoly_mulKaratsuba`:
  `DensePoly.toPoly (mulKaratsuba p q) = DensePoly.toPoly p * DensePoly.toPoly q`
...
@[simp] lemma toPoly_mulKaratsuba (p q : DensePoly R) :
    DensePoly.toPoly (mulKaratsuba p q) = DensePoly.toPoly p * DensePoly.toPoly q := by
