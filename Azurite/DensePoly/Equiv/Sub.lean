...
@[simp] lemma coeff_neg' (q : DensePoly R) (n : ℕ) :
...
private lemma ofFn_sub_coeff (p q : DensePoly R) (n : ℕ) :
...
omit [DecidableEq R] in
private lemma coeff_zero (n : ℕ) : coeff (0 : DensePoly R) n = 0 := by
  simp [coeff]
...
@[simp] lemma coeff_sub (p q : DensePoly R) (n : ℕ) :
  ...
  · -- p = 0: sub 0 q = -q
    next hps =>
    have hp : p = 0 := DensePoly.ext (Array.eq_empty_of_size_eq_zero hps)
    rw [hp, coeff_zero, zero_sub, coeff_neg']
  · split
    · -- q = 0: sub p 0 = p
      next _ hqs =>
      have hq : q = 0 := DensePoly.ext (Array.eq_empty_of_size_eq_zero hqs)
