import Azurite.DensePoly.RootBound
...
private lemma no_root_of_size_le_one
    (f : R →+* ℚ) (p : Azurite.DensePoly R)
    (hsize : p.coeffs.size ≤ 1)
    (hpne : Polynomial.map f (DensePoly.toPoly p) ≠ 0)
    (r : ℚ) (hroot : (Polynomial.map f (DensePoly.toPoly p)).IsRoot r) : False := by
  have hnd0 : (DensePoly.toPoly p).natDegree = 0 := by
    rw [DensePoly.natDegree_toPoly]; simp [Azurite.DensePoly.natDegree]; omega
  have hnd_map : (Polynomial.map f (DensePoly.toPoly p)).natDegree = 0 :=
    le_antisymm (Polynomial.natDegree_map_le.trans (by omega)) (Nat.zero_le _)
  set q := Polynomial.map f (DensePoly.toPoly p) with hq_def
...
/-- Coefficients of the mapped polynomial equal the mapped DensePoly coefficients. -/
private lemma coeff_map_toPoly (f : R →+* ℚ) (p : Azurite.DensePoly R) (i : ℕ) :
    (Polynomial.map f (DensePoly.toPoly p)).coeff i = f (p.coeff i) := by
...
private lemma cauchy_bound_case
    (f : R →+* ℚ) (hf : Function.Injective f) (p : Azurite.DensePoly R)
    (hsize : ¬ p.coeffs.size ≤ 1)
    (hpne : Polynomial.map f (DensePoly.toPoly p) ≠ 0)
    (r : ℚ) (hroot : (Polynomial.map f (DensePoly.toPoly p)).IsRoot r) :
    |r| < rootBound f p := by
  unfold rootBound
  ...
  · -- Hard case: |r| ≥ 1
    ...
    have hbound2 : ∑ i ∈ range n, |q.coeff i| * |r| ^ i ≤
      ...
      have hci : |q.coeff i| ≤
        ...
        -- DensePoly invariant: last coeff ≠ 0 in R
        ...
        have hndeg : n = p.coeffs.size - 1 := by
          have h1 : n ≤ p.coeffs.size - 1 := by
            show q.natDegree ≤ _
            calc q.natDegree
              ≤ (DensePoly.toPoly p).natDegree := Polynomial.natDegree_map_le
            _ = p.coeffs.size - 1 := DensePoly.natDegree_toPoly p
          have h2 : p.coeffs.size - 1 ≤ n := by
            show _ ≤ q.natDegree
            apply Polynomial.le_natDegree_of_ne_zero
            show q.coeff (p.coeffs.size - 1) ≠ 0
            rw [hq_def, Polynomial.coeff_map, coeff_toPoly_eq]
            simp [Azurite.DensePoly.coeff,
        ...
        have hqi : q.coeff i = f (p.coeffs[i]'hi_sz) := by
          rw [hq_def, Polynomial.coeff_map, coeff_toPoly_eq]
          simp [Azurite.DensePoly.coeff, show i < p.coeffs.size from hi_sz]
        -- q.leadingCoeff = f(p.coeffs[size-1])
        have hqlc : q.leadingCoeff =
            f (p.coeffs[p.coeffs.size - 1]'(by omega)) := by
          show q.coeff n = _
          rw [hq_def, Polynomial.coeff_map, coeff_toPoly_eq]
          simp [Azurite.DensePoly.coeff, hndeg,
            show p.coeffs.size - 1 < p.coeffs.size from by omega]
...
theorem rootBound_bounds_roots
    ...
    (p : Azurite.DensePoly R) (r : ℚ)
    (hpne : Polynomial.map f (DensePoly.toPoly p) ≠ 0)
    (hroot : Polynomial.IsRoot (Polynomial.map f (DensePoly.toPoly p)) r) :
    |r| < rootBound f p := by
