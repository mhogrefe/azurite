...
Provides tuners for `DensePoly ℤ`, `DensePoly ℚ`, and `DensePoly (ZMod n)`.
-/

open Azurite Azurite.DensePoly Azurite.Random Azurite.Benchmark

-- ── Test input generation ───────────────────────────────────────────────────

/-- Generate `n` pairs of random `DensePoly ℤ`. -/
def generateTestPairsInt (n : Nat) (meanDegree : Rat) (seed : UInt64) :
    Array (DensePoly ℤ × DensePoly ℤ) := Id.run do
  let mut g := mkDensePolyIntRandomGen meanDegree 8 seed
  let mut pairs : Array (DensePoly ℤ × DensePoly ℤ) := #[]
...
/-- Generate `n` pairs of random `DensePoly ℚ`. -/
def generateTestPairsRat (n : Nat) (meanDegree : Rat) (seed : UInt64) :
    Array (DensePoly ℚ × DensePoly ℚ) := Id.run do
  let mut g := mkDensePolyRatRandomGen meanDegree 8 seed
  let mut pairs : Array (DensePoly ℚ × DensePoly ℚ) := #[]
  for _ in List.range n do
    let (a, g') := DensePolyRandomGen.next g
    let (b, g'') := DensePolyRandomGen.next g'
    pairs := pairs.push (a, b)
...
/-- Generate `n` pairs of random `DensePoly (ZMod p)`.
    Uses `p = 65537` (a Fermat prime) for realistic modular arithmetic. -/
def generateTestPairsZMod (n : Nat) (meanDegree : Rat) (seed : UInt64) :
    Array (DensePoly (ZMod 65537) × DensePoly (ZMod 65537)) := Id.run do
  let mut g := mkDensePolyZModRandomGen 65537 meanDegree seed
  let mut pairs : Array (DensePoly (ZMod 65537) × DensePoly (ZMod 65537)) := #[]
  for _ in List.range n do
    let (a, g') := DensePolyRandomGen.next g
    let (b, g'') := DensePolyRandomGen.next g'
    pairs := pairs.push (a, b)
...
def benchThresholdGeneric [Ring R] [DecidableEq R]
    (threshold : Nat) (pairs : Array (DensePoly R × DensePoly R)) : IO (UInt64 × Nat) := do
  let t0 ← monoNanos
...
def benchThresholdMedianGeneric [Ring R] [DecidableEq R]
    (threshold : Nat) (pairs : Array (DensePoly R × DensePoly R)) : IO UInt64 := do
...
partial def goldenSectionSearchGeneric [Ring R] [DecidableEq R]
    (lo hi : Nat)
    (tolerance : Nat)
    (pairs : Array (DensePoly R × DensePoly R))
    (verbose : Bool := true)
    : IO Nat := do
...
private def reportResult [Ring R] [DecidableEq R]
    ...
    (pairs : Array (DensePoly R × DensePoly R)) : IO Unit := do
  IO.eprintln ""
...
/-- Tune Karatsuba threshold for `DensePoly ℤ`. -/
def tuneKaratsuba
    (lo : Nat := 4) (hi : Nat := 256)
...
/-- Tune Karatsuba threshold for `DensePoly ℚ`. -/
...
/-- Tune Karatsuba threshold for `DensePoly (ZMod 65537)`. -/
...
def tuneKaratsubaAll
  ...
  IO.eprintln s!"  DensePoly ℤ:          optimal threshold = {bestZ}"
  IO.eprintln s!"  DensePoly ℚ:          optimal threshold = {bestQ}"
  IO.eprintln s!"  DensePoly (ZMod 65537): optimal threshold = {bestZM}"
  IO.eprintln "══════════════════════════════════════════"
