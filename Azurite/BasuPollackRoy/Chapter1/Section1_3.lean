...
and `mkTRemsNode`), and the degree formulas `degFormula`,
`degEqFormula`, `degNeqFormula` (Notation 1.18) are defined in
`Azurite.BasuPollackRoy.Chapter1.Section1_3.{Specialize,SplitLast,Projection,Fiber,SignedPseudoRemainder,Truncate,Tru,TRems,DegFormula}`. -/
