...
/-!
### Splitting the last variable

For posgcd-style manipulations we need the opposite view of `specialize`:
instead of evaluating `Y` at a concrete point in `C^k`, we keep the
`Y`-variables **symbolic** and split off the last variable `X` as the
polynomial variable. The result is a univariate polynomial in `X` with
coefficients in `D[Y₁, …, Y_k]`. Unlike `MvPolynomial.finSuccEquiv`
which splits the *first* variable, `splitLast` splits the *last*.
-/
...
at `Fin.snoc y x`. -/
theorem aeval_snoc_eq_eval_splitLast [Algebra D C]
