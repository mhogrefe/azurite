...
/-- **BPR Corollary 2.49.** Every root of `P` is a virtual root of `P`, the
    ...
    The proof parallels the induction in `lemma_2_48`, but invokes the
    Proposition 2.46 interpretations (`virtualMultiplicity_derivative_of_root`
    and `virtualMultiplicity_diff_of_not_root`) directly — these are the
    building blocks that Lemma 2.48 itself is based on. -/
theorem corollary_2_49
  ...
  | succ n ih =>
    ...
    by_cases hPc : P.eval c = 0
    · -- Root case: `μ(P,c) = μ(P',c) + 1` and `v(P,c) = v(P',c) + 1` (Prop 2.46(1)).
      refine ⟨k', ?_⟩
      have h46 := virtualMultiplicity_derivative_of_root hIVP hP hdP hPc
      ...
      omega
    · -- Non-root case: `μ(P,c) = 0`; use Prop 2.46(3) trichotomy on `ν` parity.
      have hrm : P.rootMultiplicity c = 0 :=
        Polynomial.rootMultiplicity_eq_zero hPc
